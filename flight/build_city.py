...
JOBS = [{
    ...
    #   反推（管線近似乘法）：val 0.41/0.659≈0.62、sat 0.20/0.283≈0.70。
    #   ⚠ 建完要**再量一次成品**再修（管線不是純乘法）。
    'sat': 0.70, 'val': 0.62,
    'towers': 0.55,
    # ⚠ **不開 `dropGreen`**：那是 Ray 為東方泊地點名的（「把東泊的綠色拿掉，
    #   純礙事」）—— 這張的綠是城自己的農地與林帶（外環一整圈），拿掉等於把城
    #   砍成只剩市街核心。沒有人要求就不要動。
    ...
    # ⚠ `planW` 700：與北方泊地／東方泊地同級（都是港市），碼頭外緣＝中心西 17.5 格。
    # ⚠ 這三個值與 index.html 的 SETTLEMENTS **必須一致**（見檔頭）。
    'mx': 577, 'my': 243, 'planW': 700, 'planRot': -1.5708,
    'landmarks': [],
}]
