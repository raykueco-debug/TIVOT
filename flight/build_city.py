...
JOBS = [{
    ...
    # ── 東方泊地（ver -1177；美術 session 交件 `Eastport_topdown.png`，Gemini 生）──
    # 19 世紀地中海港都：密集赤陶瓦舊城、窄巷網、碼頭邊一排倉庫、城緣的果園菜畦。
    # ⚠ 正俯視（unsquash 1.0，同北方泊地／夏爾村）——照預設 1.60 拉伸會變長條。
    #
    # ⚠⚠⚠ **`planRot` 是量出來的，不是選的**：
    #   · 插畫的**碼頭與倉庫在下緣**（`v=+1` 那一側）。
    #   · 地形實測：(2030,590) 是一個**朝東的海灣**（西/北/南三面陸、東面海），
    #     y≈590 那一列的岸線在 **x≈2019**。
    #   · 轉換式是 `wx = mx + (lx·cos − ly·sin)/S`、`wy = my + (lx·sin + ly·cos)/S`
    #     ⇒ 要讓「圖的下緣」朝 +x（東），得 `sin = −1、cos = 0` ⇒ **planRot = −π/2**。
    #   順帶：圖的上緣→西（內陸）、右緣→北、左緣→南。右上角那片果園因此落在
    #   內陸西北，而**被切掉的那兩邊（上、右）也剛好都朝內陸** —— 切口不在水岸邊。
    #
    # ⚠ `mx/my`：碼頭那一條要壓在岸線上。不透明區到 y≈96% 才收（下面那截是碼頭），
    #   ⇒ 碼頭離圖心 ≈0.92·hh = 322 世界單位 = 16.1 地圖像素
    #   ⇒ mx = 2019 − 16.1 ≈ **2003**，my = 590。
    # ⚠ `planW` 700：與北方泊地同級（都是泊地），texel 尺寸也因此一致（700/440）。
    # ⚠⚠ 色調（**ver -1178 第二版重調**，Ray：「圖太密顏色太深，讓 gem 畫一個更適合
    #   放在飛行地圖的圖」）：新圖**本來就亮很多** —— 來源 V 0.772／S 0.268，
    #   而第一版是 V 0.483／S 0.40。沿用第一版那組（0.55/0.95）會變成一塊白斑。
    #   ⚠⚠⚠ **不要直接回預設** —— 預設（0.70/0.86）是對著**北方泊地那種亮度**
    #     （來源 V 0.483）訂的，套在 V 0.772 的圖上會算出 V≈0.66，比成品目標亮六成。
    #   目標取**成品**（貼到地形上的那一張）的實測值，不是來源：
    #     北方泊地成品 V 0.401／S 0.135、帝都成品 V 0.422／S 0.235。
    #     地中海港都該比北國港埠暖、但不該比帝都豔 ⇒ 取 **V 0.41／S 0.20**。
    #   ⚠ 管線**不是純乘法**（逐類別對齊會把綠地與水拉向地形色，縮圖與 webp 也會動
    #     統計）。用第一版的實測反推係數：V_out ≈ V_src×val×0.88、S_out ≈ S_src×sat×1.17
    #     ⇒ val = 0.41/(0.772×0.88) ≈ 0.60、sat = 0.20/(0.268×1.17) ≈ 0.64。
    #   量了成品再修一次（0.60/0.64 出來是 V 0.441／S 0.177，比目標亮 7%、淡 12%）
    #     ⇒ 現行 **val 0.56／sat 0.72**。
    #   ⚠ 改圖就要重量這兩個數字 —— 它們是對**那一張**的統計解出來的，不是通用值。
    # ⚠ `towers`：這張沒有高度資訊，不開的話每一棟一樣高、讀起來是平的（-1114 的教訓）。
    # ⚠ 三個值（mx/my/planW/planRot）**必須與 index.html 的 SETTLEMENTS 一致**（見檔頭）。
    'src': 'Eastport_topdown.png',
    'unsquash': 1.00,
    # ver -1180（Ray：「太灰太糊了，不要那麼糊」）：
    #   · **太灰** → 色調放回去。-1178 為了對齊兩座基準城壓到 S 0.195／V 0.412，
    #     那是「跟北方泊地一樣灰」—— 但這是**地中海**港都，本來就該比北國港埠豔。
    #     目標改成 **S≈0.26／V≈0.47**（比帝都成品的 0.235 再暖一點）。
    #   · **太糊** → `maxdim`。來源是 1024，預設 440 等於把它縮掉一半以上；
    #     檔頭那句「440 讓細節密度接近地形」是對**等角視的舊插畫**講的，
    #     而 Ray 已經為了同一個理由把夏爾村推到 660（ver -1115）。
    #     這裡直接給 **1024＝不縮**：再高也沒有來源了，而且不縮就沒有重取樣的糊。
    'sat': 0.96, 'val': 0.64,
    'maxdim': 1024,
    # ⚠⚠ **門檻要跟著 `maxdim` 放大**（見 BLK_* 那一段的①：夏爾村踩過 259→675 塊、
    #   fps 腰斬）。440→1024 ⇒ 面積 ×5.41、長度 ×2.33：
    #   `blkMin` 20→108、`blkSplit` 900→4870、`blkCell` 30→70。
    #   不放大的話實測會切出 **1419 塊**（帝都才 344）。
    'blkMin': 108, 'blkSplit': 4870, 'blkCell': 70,
    'towers': 0.55,
    ...
    'mx': 2003, 'my': 590, 'planW': 700, 'planRot': -1.5708,
    'landmarks': [],
}, {
    # ══ 拉芬斯達爾城 Ravnsdal（ver -1193，美術交件 ver -1163）══════════════
    # 交件單：flight/city/_ravnsdal_plan.md　規格：resources/map/_ravnsdal_spec.md
    # 硬條件美術端都驗過了：輪廓內破洞 0px、四邊不碰框（x 32~1228 / y 18~1227）、
    # 無文字／圖例、沒畫海。⚠ 我這邊複驗過 alpha 與色調，數字對得上。
    'src': 'Ravnsdal_topdown.png',
    'unsquash': 1.00,          # 正俯視，不必反投影
    # ⚠⚠ **`maxdim` 給 1254＝不縮**（同東方泊地那一段的理由）：來源就是 1254，
    #   預設 440 會把它縮掉七成，那正是 Ray 退過兩次的「太灰太糊」。
    'maxdim': 1254,
    # ⚠⚠ 門檻跟著 `maxdim` 放大（見 BLK_* 那一段的①）：440→1254 ⇒
    #   面積 ×8.12、長度 ×2.85 ⇒ 20→162、900→7310、30→86。
    #   不放大就會把一棟房子切成一堆碎塊（夏爾村踩過 259→675 塊、fps 腰斬）。
    'blkMin': 162, 'blkSplit': 7310, 'blkCell': 86,
    # ⚠ 色調：**目標訂在成品**（§6.7.5 步驟 5），不是來源。
    #   來源 V 0.659 / S 0.283（美術報 0.660/0.283，我複驗一致）。
    #   基準：北方泊地成品 V 0.401/S 0.135、帝都成品 V 0.422/S 0.235 ——
    #   這是一座有紅瓦的正規城市，取兩者之間偏帝都：**目標 V≈0.41 / S≈0.20**。
    #   ⚠⚠⚠ **基準要取「Ray 簽過的那一張」，不是最舊的那幾張**（ver -1194 修）：
    #     -1193 我瞄準北方泊地(0.401/0.135)與帝都(0.422/0.235)之間 → 成品
    #     V 0.423 / S 0.196，Ray 一看就是「**太糊了，而且掉水裡了**」——
    #     與他當初退東方泊地那一版用的字（「太灰太糊」）是同一句。
    #     **東方泊地 V 0.488 / S 0.271 才是他驗收過的那一張**；北方泊地與帝都
    #     是更早的舊圖，拿它們當目標就是照著被退過的方向調。
    #   反推：val 0.62×(0.488/0.423)≈0.72、sat 0.70×(0.271/0.196)≈0.97。
    'sat': 0.97, 'val': 0.72,
    'towers': 0.55,
    # ══⚠⚠⚠ `dropGreen` **要開**（ver -1194 改；-1193 我判斷錯了）══
    #   -1193 的理由是「這張的綠是城自己的農地與林帶，沒有人要求就不要動」——
    #   **那個理由只看了插畫，沒看它貼到地形上長什麼樣**。實測 A/B（同一個取景、
    #   同一個畫質檔）：東方泊地清楚可讀，拉芬斯達爾**整片化在森林裡** ——
    #   城的外環是綠的、地圖那一帶也是綠的，邊界因此消失，看起來就是一團糊。
    #   那正是 Ray 當初對東方泊地說「把綠色拿掉，**純礙事**」的同一件事。
    ...
    # ══⚠⚠⚠ 落點與旋轉（ver -1193）══════════════════════════════════════
    # `mx/my` ＝ **Ray 指定的 (577,243)**，不是搜出來的：位置定了之後要問的是
    #   另一題 —— 「**在這一點**，哪一個 planRot 讓碼頭對著水」。
    #   （`place_city.py` 全圖搜出來的是「哪裡最像」，會誠實地把城搬到大陸另一頭；
    #    ver -1193 因此給它加了 `--near`。）
    # ⚠ 那一帶的水文（實測，地圖像素）：
    #     · 一條南北向的河在城心**東邊約 23 格**（由北方 y≈223 流下、向西南彎）
    #     · 一條東西向的大河在城心**南邊約 20 格**（y≈263，x 543~611）
    # ⚠⚠ `planRot = −π/2` 把圖轉成：
    #     **左緣（碼頭）朝南** → 碼頭外緣落在 y≈260，離南邊那條大河只有 3 格
    #       ＝ Ray 的「著水靠岸」與美術點名的「碼頭那一側朝向南方那條河」都成立
    #     **圖的下緣朝東** → 城裡那條河的出口對著東邊那條南北向的河（不是斷頭河）
    #     **圖的右緣（鐵路）朝北** → 鐵路往內陸走
    #   ⇒ 城坐在兩條河的交角裡：南岸停船、東邊接河、北邊出鐵路。
    # ⚠⚠ `planW` **800**（ver -1194 由 700 改）：700 的城在地圖上只有 35 格寬，
    #   細節擠成一團 —— 那是 Ray 說「太糊」的另一半。800 ＝ 與**聖索菲亞同級**
    #   （同樣是 12 格的正規城市），街廓密度也由 5.71 降到 4.38 塊/100世界單位²
    #   （聖索菲亞 3.23／北方泊地 3.67／東方泊地 6.94）。
    #   ⚠⚠ **順便對上碼頭**：hw = 800/2/20 = **20 格** ⇒ 碼頭外緣落在 y = 243+20
    #   = **263**，正好是南邊那條大河的中心線 —— 700 時差 2~3 格、900 會越過去。
    # ⚠ 這三個值與 index.html 的 SETTLEMENTS **必須一致**（見檔頭）。
    'mx': 577, 'my': 243, 'planW': 800, 'planRot': -1.5708,
    'landmarks': [],
}, {
    # ══⚠⚠⚠ 卡耶爾山谷（ver -1244，Ray：「山谷是這張 flight/city/Canyon_topdown.png」）══
    #   **它不是城，是地形** —— 美術交件時特別點名的三件事，接的時候全部照辦：
    #   ① 不進 `SETTLEMENTS`（進了會被 buildRoads 連一條路通到死谷）——
    #      改由 `PLACES` 那一列帶 `plan`，取樣器兩邊都吃（ver -1243 的 `cityPlates`）。
    #   ② **不要 podium**：那道谷是 -1235~-1237 手刻的（`TERRAIN_VALLEY`），
    #      整平就等於把谷填掉，平面圖會貼在一塊平地上。
    #   ③ `nomass` —— 谷底只有骸骨、斷橋、石壇，那些不是多邊形量體表達得了的東西。
    'src': 'Canyon_topdown.png',
    # ⚠⚠⚠ **正俯視 ⇒ `unsquash` 一定要給 1.00**（同北方泊地／夏爾村／拉芬斯達爾）。
    #   預設是 **1.60**（給等角視反投影用的），沒給的話這張 733 寬的谷會被壓成
    #   **458 寬** —— 谷會變成一條細線，而且與地形對不上。第一次建就踩到了。
    'unsquash': 1.00,
    'nomass': True,
    'nowater': True,          # 谷底是乾河床，沒有水面要挖
    # ⚠⚠ `maxdim` 給**原生長邊 1536，不要用預設 440**（東方泊地與拉芬斯達爾的教訓：
    #   440 會把來源縮掉七成 ⇒ Ray 退過兩次的「太灰太糊」）。`nomass` 所以
    #   blkMin/blkSplit/blkCell 用不到。
    'maxdim': 1536,
    # ══ 色調（§6.7.5 第 5 步：先反推一組、建完量成品再修一次）══
    #   來源實測 V 0.535 / S 0.123（1024×1536、alpha>8 佔 39%）。
    #   ⚠⚠ **這一張的目標不是城的基準**：它要與**谷底的棕石地**接得上 ——
    #     那是 -1237 我照 Ray「谷不要草地，用棕石地」鋪的 `rock.col [134,108,78]`
    #     （地圖底色 V≈0.52 / S≈0.36）。拿東方泊地(0.488/0.271)那組去瞄，
    #     貼上去會比它腳下的地淡一階、邊界整圈看得見。
    #   ⇒ 目標 **V≈0.50 / S≈0.30**；反推 val 0.50/(0.535×0.965)≈0.97、sat 0.30/0.123≈2.4。
    #     ⚠ sat 2.4 會把灰岩推成假的暖色 —— 先給 **1.60**（產出 S≈0.20），
    #       建完量成品再決定要不要再推。
    'sat': 2.25, 'val': 0.92,
    'dst': 'canyon_plan.webp',
    'hdst': 'canyon_h.webp',
    'mdst': 'canyon_mass.webp', 'jdst': 'canyon_mass.json',
    # ⚠ `mx/my` 是**平面圖自己的中心**（index.html 那一列的 `planX/planY`），
    #   不是地點座標 (985,858)：那一點是降落點與名牌，不能為了貼圖去搬。
    #   `planW 1200` ⇒ 1200×1800 世界單位（60×90 地圖格），蓋谷的南段，
    #   0.85 texel/世界單位（與帝都同級）。
    # ══⚠⚠⚠ 尺寸與落點（ver -1245，Ray：「成了平貼的了，不用整那麼大，
    #   比船大1倍就好, 放在降落座標」）══════════════════════════════════════
    #   -1244 是「蓋滿整條谷」（planW 1760＝88×184 格）—— 攤成那麼一大片，
    #   它就不再是一個**地點**，而是一張鋪在地形上的貼紙。
    #   ⇒ 改成**降落點上的一小塊**：
    #     · 落點回到 `PLACES` 那一列自己的 (985,858)（拿掉 planX/planY）。
    #     · 大小 ＝ **船的兩倍**。船的世界尺寸是**量出來的**（ver -1245）：
    #       量船在 2D 畫布上的非透明外框（113×165 CSS px），再乘那一點的
    #       「每 CSS 像素幾個世界單位」（2z/BW × BW/W，z 由 drawShip 寫死的
    #       螢幕 y=VIEWH*0.76 反推）⇒ **224 × 326 世界單位**。
    #       取寬的兩倍 ⇒ **planW 450**（成品 450×943 世界＝22×47 地圖格）。
    #   ⚠ 順帶解決解析度：733/450 = **1.63 texel/世界單位**（帝都 0.82）——
    #     -1244 那個 0.42 的問題自己沒了。
    'mx': 985, 'my': 858, 'planW': 450, 'planRot': 0.0,
    'landmarks': [],
}]
