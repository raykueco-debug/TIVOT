...
PLACES = [
    ...
    (1098, 798, "夏爾村",       "帝國・村落"),
    # ver -1134（Ray：「在 735 196 放置地點伊甸古墓」）：34 格的迷宮型遺跡。
    # ⚠ 只標**看得見的地點**：兩座隱藏遺跡（無名遺跡／石製遺跡）**刻意不列**
    #   —— 這張參考圖不該劇透隱藏點（index.html 那邊的註解同一條）。
    (723,  137, "伊甸古墓",     "遺跡"),   # ver -1147 壁腳（見 index.html 那一筆的說明）
    (1972, 656, "貝利薩爾遺址", "遺跡"),   # ver -1156 崩壞的哥德式古堡（降落點另在 1951,651）
