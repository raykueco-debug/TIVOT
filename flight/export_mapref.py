...
PLACES = [
    ...
    (985,  858, "卡耶爾山谷",   "險地"),
    (1516, 150, "北方泊地",     "泊地"),
    (2003, 590, "東方泊地",     "泊地"),   # ver -1177 名牌跟著城走（build_city eastport）
    # ver -836：那座「備著」的無名村落正名為夏爾村，座標對回 index.html（-835 Ray 定點）。
    (1098, 798, "夏爾村",       "帝國・村落"),
    # ver -1134（Ray：「在 735 196 放置地點伊甸古墓」）：34 格的迷宮型遺蹟。
    # ⚠ 只標**看得見的地點**：兩座隱藏遺蹟（無名遺蹟／石製遺蹟）**刻意不列**
    #   —— 這張參考圖不該劇透隱藏點（index.html 那邊的註解同一條）。
    (723,  137, "伊甸古墓",     "遺蹟"),   # ver -1147 壁腳（見 index.html 那一筆的說明）
    (1972, 656, "貝利薩爾遺址", "遺蹟"),   # ver -1156 崩壞的哥德式古堡（降落點另在 1951,651）
    (183,  762, "羅賽爾廢城",   "遺蹟"),   # ver -1746 Ray 指定、-1753 正名；在羅賽爾禁航區內
    (544,  595, "里朋莊園",     "莊園"),   # ver -1755 Ray 指定；聖索菲亞郊外
    # ver -1191（Ray 指定）：拉芬斯達爾城（Ravnsdal）。城還沒建（只有名牌），
    # 規格見 resources/map/_ravnsdal_spec.md。實測 h=73（陸），南方約 20 格有水面。
    # ⚠ 城的插畫落地之後名牌要跟著城走（§6.7.5），這裡也要一起改。
    # ⚠⚠ 驗地形別拿灰階直接比 CLOUD_H —— 那是世界高度，灰階門檻是 44/520*255＝21.6
    #    （見 index.html 那一筆的說明；-1191 踩過一次）。
    (577,  243, "雪都瓦恩霍姆", "港市"),   # ver -1491 改名（原「拉芬斯達爾城」）
