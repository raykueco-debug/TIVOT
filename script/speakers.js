     ...
     ⚠⚠ `standCm` 140 → **152**、`fx` **量到 0.363 但填 0.332**：
       ...
       ⚠⚠ 這是**刻意偏離實測值的位移**，不是量錯。兩件事要記住：
         ...
         ② **同一個位移要套到每一張差分**（見下面 `dead` 的說明）——
            ...
            會讓她在斷氣那一拍橫向跳一大格。 */
  natalia:{ cm:150, standCm:152, eye:0, fx:0.310, top:103, bot:1535,
    ...
    /* ⚠⚠ **`fx` 是「臉在**這張圖**裡的橫向位置」，不是螢幕位置** —— 所以**不能抄**。
       ...
       ⚠ 那「往右移一點」的手調怎麼辦？**位移是加在各自的實測值上的同一個差**：
         dying 0.363 − 0.053 = 0.310（Ray 定的），所以 dead 也要 0.594 − 0.053 = **0.541**。
         要再左右挪就**兩個一起加同一個差**，不要只動一張。 */
    dead: { src:'resources/SI/NPC_Natalia_SI_dead.webp', top:47, bot:1535, fx:0.541 },
  } },
