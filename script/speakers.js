...
export const ART = {
  ...
  /* ⚠ ver -641 換過圖並**重量**：兩張的數字與上一版對調了（`dying` 103/0.363、
     ...
     ⚠⚠ `standCm` 140 → **152**、`fx` **量到 0.363 但填 0.332**：
  ...
  natalia:{ cm:150, standCm:152, eye:0, fx:0.363, fxShift:0.012, top:103, bot:1535,
    ...
    /* ⚠⚠ **躺著的人要對「身體」不是對「臉」**（ver -646，Ray：「dead 的時候圖跑位了，
       而且也沒有出 dead 立繪」）。取景公式錨的是**臉**（`fx`）—— 對站姿是對的，
       但這兩張是同一個人**癱在地上**的兩個角度：`dying` 的臉在圖上 0.363、
       `dead` 因為頭歪到一邊而跑到 0.594。**把臉對齊 ＝ 身體整個橫移 120 幾個像素**，
       讀起來就是「她瞬間滑到旁邊」，而且大半被推出畫面外（看起來像沒出來）。
       ⚠ 作法：`fx` 仍是**實測的臉位置**（不要動），改用**這一張自己的 `fxShift`**
         （`frameOf` 會讓 expr 的覆蓋掉角色層的）把**肩**對到與 `dying` 同一條線：
           肩在圖上第 289 欄（上 45% 的最左不透明欄，`dying` 是 168），
           解 `fx+fxShift = W×0.24 ÷ (縮放×1024) + 289/1024` → 0.5013
           → `fxShift = 0.5013 − 0.594 = −0.093`。
       ⚠ 兩張的縮放不同（`bot−top` 1432 vs 1488），所以不能直接抄角色層那個 +0.012。 */
    dead: { src:'resources/SI/NPC_Natalia_SI_dead.webp', top:47, bot:1535, fx:0.594, fxShift:-0.093 },
  } },
