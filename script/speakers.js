...
export const ART = {
  ...
  natalia:{ cm:150, standCm:152, eye:0, fx:0.300, top:103, bot:1535,
    ...
    /* ⚠⚠ **`fx` 是「臉在**這張圖**裡的橫向位置」，不是螢幕位置** —— 所以**不能抄**。
       ...
       不同的地方：dying 量到 0.363、dead 量到 0.594）。
       ⚠⚠ 那「往右移一點」的手調怎麼辦？**位移是加在各自的實測值上的同一個差**。
         ⚠ **實測值**：dying **0.363**、dead **0.594**（`tools/measure_si.py` 量的，
           下面兩個 `fx` 是「實測值 − 位移」之後的結果，不是實測值本身）。
         ⚠ **改法**（Ray：「那我手動調第一張，你再調第二張吧」）：
           動上面那個 `natalia.fx`（往左加、往右減），然後
             **`dead.fx = 0.594 − (0.363 − natalia.fx)`**
           現行：natalia.fx = 0.3 → 位移 0.063 → dead.fx = 0.531。
         ⚠ 只動一張的話，她在斷氣那一拍會橫向跳一格。 */
    dead: { src:'resources/SI/NPC_Natalia_SI_dead.webp', top:47, bot:1535, fx:0.531 },
  } },
