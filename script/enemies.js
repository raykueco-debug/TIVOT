...
export const ENEMIES = {
    /* ⚠⚠ 敵人卡的兩個統一欄位（ver -495，Ray 指定「敵人卡統一加上」）：
       ...
       `boss`          ＝ **這一隻是不是 Boss**（1/0，ver -1024，Ray：「為敵人卡加上
                         boss=1 或 0，boss 經驗 2 倍錢 2.5 倍」）。
                         倍率在 `config.rating.bossMul`（鐵律 1），判定與相乘只有
                         `inspector` 的 `bossMul()` 一處（鐵律 7）。
                         ⚠ 連戰整段以**收段那一場**（帶 `sessionEnd` 的那隻）為準
                           —— 結算是那一場的事（§0.5：局＝一次結算）。
                         ⚠ 它與 `state.inIntruderFight`（挑戰的亂入 Boss）是兩件事：
                           那是試玩版那條線的旗，這是**卡上的性質**。
    ...
    nemo: {
      /* ⚠ ver -1570（Ray：「尼莫初戰時不要顯示名字，用『？？？』，評價也是」）——
         這一場是他**初登場**，玩家還不知道他是誰（腳本那一段也是 `NEMO_X`＋剪影）。
         ⚠ 結算副標（「？？？ 已擊敗」）讀的是同一格，所以「評價也是」自動成立。
         ⚠ 這張卡**只有 `lk_nemo` 在用**；日後他再上場要顯示本名，
    ...
    np_boss: {
      name:'背負祭壇者',
      story:1, counterStagger:1, boss:1,   // ← 北方泊地掃盪的收段場（教堂，Ray 點名）
      Ganymede:0,   // 主武器（普攻）的增傷／減傷：正=增傷、負=抗性減傷（加法，同副武器那三把）
    ...
    /* ══ 瓦礫中的紫黑之爪（ver -595（-893 前用詞），Ray 交稿）══ 教堂那一場之後的真 BOSS，
    ...
    np_claws: {
      name:'紫黑之爪',
      story:1, counterStagger:1, boss:1,   // ← 紫黑之爪（真 BOSS，Ray 點名）
      Ganymede:0,   // 主武器（普攻）的增傷／減傷：正=增傷、負=抗性減傷（加法，同副武器那三把）
      weaponMod:{ '重機槍':[0,0], '霰彈槍':[0,0], '萊福槍':[0,0] },   // 每把＝[傷害, 迴避]：傷害 正=增傷/負=抗性減傷；迴避＝額外 miss 率(0~1)。都加法(0.1＝+10%)，預設 [0,0]
      openAssault:[1,2],   // 登場第一發大絕的延遲（秒，隨機範圍）；預設 [1,2]。改小＝一登場就攻擊、改大＝緩一下
      ult:{ on:0, hp:40, count:4, atk:20, gap:0.4, cd:4 },   // 大絕：on=1 才啟用（見檔頭格式說明）
    ...
    centipi: {
      name:'巨型蜈蚣',
      story:1, counterStagger:1, boss:1,   // ← 蜈蚣（劇情 BOSS，Ray 點名）   // 劇情戰／反擊硬直（ver -495，統一欄位，見 enemies 檔頭）
    ...
    gk_seal: {
      ...
      /* ⚠⚠ **倒地差分**（ver -1701／-1703）：圖、放大倍數、下移、著地線（圖內比例）。
         ...
         胸骨在圖的 0.71 ⇒ 放大 2.3 倍後，下移 0.6 讓那一條落在演出區下緣附近（楣的上方）。 */
      down:{ image:'enemy_gk_seal_down', scale:2.3, shiftY:0.6, groundV:0.71 },
      ...
      entrance:'sfx_saint',
      /* ⚠⚠ **戰鬥中降臨＝龍吟＋降臨音兩聲同時**（ver -1703，Ray：「戰鬥中再降臨是 deerroar
         ＋降臨音兩個音效同時，不要再有其他 se」）—— 只在**戰鬥裡跑降臨**的那幾場響
         （著地那一刻，`enemy.js` 的 landT）；推棺那一場不在戰鬥裡降臨，所以不會疊到劇情那一聲。 */
    ...
    /* ══════════════════════════════════════════════════════════════════════
       伊甸古墓の雜兵 26 隻 ＋ 王座間 2 隻（ver -1578）
       —— 美術 ver -1556 盤點的「圖入庫了但沒接」那 28 張
       （清單：`resources/enemy/_enemy_card_intake.md`）
       ──────────────────────────────────────────────────────────────────────
       ⚠⚠⚠ **這 28 張整批是「佔位卡」，一格真值都沒有。** Ray 的指示是
         「**數值全部先用王座徘徊者追擊型態**」⇒ 逐格照抄 `bl_dragon_chase`
         （那一張自己又是照抄 `sf_tiger` 的）。逐張的真值等他後續給。
       ⚠⚠ **照抄的不只有數值，連下面這幾格也是照抄的 —— 它們多半不對，
         給真值的時候要一起看**（寫出來，免得下一個人以為那是決定過的）：
       ...
       ⚠⚠⚠ **現在一隻都遇不到，那是刻意的**（兩邊都還沒接，見 §四的但書）：
         ① `config.js` 的 `ASSETS` 那一段**註解著**（照聖遺物系十隻的作法）——
            登記進去就會進開機第二段的背景預載，這 28 張合計 **6.70 MB**，
            而現在遊戲裡一隻都遇不到，手機冷啟動白揹那 6.7 MB。
         ② **沒有任何戰鬥卡或刷怪池指到它們**（`config.battles`／`wildSpawn` 一個字沒動）。
         ⚠⚠ **要放出來時兩邊一起**：只放①＝多載 6.7 MB 但遇不到；
           只放②＝`asset()` 回空字串、怪沒有立繪，**而且畫面上不會有任何錯誤訊息**
           （-929 那隻教堂 Boss 踩過）。
       ⚠ **三層分區與中文名是美術暫譯／暫推的**（那份清單自己標明了）——
         以 Ray 的命名為準，改名時 `name` 與清單兩邊一起改。
       ⚠ `bg` 只是**退路**：城鎮插入戰交棒時會被 `state.battleBg`（玩家站的那一格）
         蓋過去（ver -592）—— 這裡逐層給一張，只為了「萬一沒被蓋到也不是空背景」。
       ⚠ **`entrance` 這一格在每一張卡裡只出現一次**（ver -1434 那個坑：
         同一個物件字面量裡同名鍵後者勝，`entrance:null` 排在後面會把音效整個吃掉、
         而且沒有任何錯誤訊息）。
       ══════════════════════════════════════════════════════════════════════ */
    ...
    /* ── 貝利薩爾古堡・王座間（⚠ 這兩張還沒有人指到 —— 線上的 `bl_dragon_throne` 用的是 `mon_dragon_v1_unsealed`。要不要換成這兩張、怎麼分型態是**設計決定**，等 Ray） ── */
    dragon_throne_awakened: {
      name:'王座徘徊者',
      story:1, counterStagger:1, boss:0,
      Ganymede:0,
      weaponMod:{ '重機槍':[0,0], '霰彈槍':[0,0], '萊福槍':[0,0] },
