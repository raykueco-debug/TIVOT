...
export const INTRUDE = {
  /* ⚠⚠⚠ **`default` ＝不分場次都會亂入**（ver -958，Ray：「索拉娜在評價 CD 時沒有亂入」
     「索拉娜的亂入是指 cd 評價時在蕾娜評完後她會跑出來說話」）。
     實測機制是好的（`sv_wild` 拿 D 她確實會出來）—— 沒出來的原因是**只有那一場有稿**。
     ⚠ **台詞要 Ray 給**（同護符與九星配方：卡沒到就空著，不自己發明）：
       下面那兩句是村戰專屬的（「這次算平手」指的是那一場），搬來當通用的會不通。
     ⚠ 補上去之後**每一場** C／D 都會有她 —— 包含她不在隊上的場次，
       要不要限制（例如只有她在隊時）也一起說。
     格式與下面同：`{ name, portrait, text, delayMs? }`。 */
  // default: { C:{…}, D:{…} },
  sv_wild: {
