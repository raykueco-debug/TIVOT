/* ══════════════════════════════════════════════════════════════════════
   ...
   ──────────────────────────────────────────────────────────────────────
   起點 **1908年10月11日 11:00**（Ray 指定：踏進城鎮探索的那一刻；
   ver -384 把時刻由 14:00 改成 11:00，ver -1324 把日期由 6/13 改成 10/11）。
...
/* 起點：1908-10-11 11:00（ver -1324，Ray：「把遊戲起始時間改成 10月11日」）。
   ⚠ 只有這裡是計算點（鐵律 7）—— 別處要日期一律問這支。
   ⚠⚠ `flight/index.html` 有**第二份**（那一頁是非 module，import 不到這裡）——
...
/* ══ N 天之後的日期（ver -953，Stage8 的「兩個月內完成任務」）══
   ...
   ⚠⚠ **不印年**（ver -1033，Ray：「x月y日不用寫年，月日就好」）——「兩個月內」
     講的是眼前的期限，講話的人不會把年份唸出來。⚠ 與 `dateText()`（情境卡的
     `{DATE}`）是**兩支**：那一支是報時間地點的字卡，年份照留。 */
