...
/* ══════════════════════════════════════════════════════════════════════
   ...
   ⚠ 分店只換**背景與地名**，不是另一個節點 —— 出口、營業時間、店主都還是那一格的。
   ⚠ 背景名＝`<節點的 bg>_<scenes[].bg>`（時段差分照吃）；**載不到就退回節點原本那張**
     （`bgFor` 吃一串基底名，見那一支）—— 所以圖還沒交也不會變成空畫面。
   ⚠ 哪一格是餐飲街寫在**城**上（`dining`），不寫死節點 id：
       `dining:{ node:'tavern', scenes:true }`  ← 換分店（北方泊地）
       `dining:{ node:'tavern' }`               ← 只當「可以碰到人的一格」，不換分店
     帝都的餐酒館維持原樣：它有 stage 0 自己的戲與自己的一張圖（`Capital_Bistro`），
     四張分店圖到齊之前換上去只會退回同一張、地名卻變了。要開就把 `scenes:true` 補上。
   ══════════════════════════════════════════════════════════════════════ */
export const DINE = {
  fallback: 'bar',       // Ray：無女伴、該場景無分配角色 → 酒吧
  scenes: {
    cafe:       { name:'咖啡廳', bg:'cafe' },
    restaurant: { name:'餐廳',   bg:'restaurant' },
    dessert:    { name:'甜品店', bg:'dessert' },
    bar:        { name:'酒吧',   bg:'bar' },
  },
};
...
export const TOWNS = {
  capital: {
    ...
    /* ══ 餐飲街那一格（ver -575）══════════════════════════════════════════
       帝都的餐酒館**不換分店**：它有 stage 0 自己的戲、也只有一張圖（`Capital_Bistro`）。
       寫著它是為了讓女角上得了那一格（外出時可以在這裡碰到人，見 OUTING）。
       四張分店圖到齊了就把 `scenes:true` 補上，程式端不必動。 */
    dining:{ node:'tavern' },
  ...
  northport: {
    ...
    /* ══ 餐飲街換分店（ver -575，Ray：「大城地圖已經規則化，餐飲街方向會出什麼場景
         取決於同行女伴，或者當下是誰在那個區域」）══
       ⚠ 圖還沒交：`Northport_tavern_<bar|cafe|restaurant|dessert>` 四張。
         載不到就退回 `Northport_tavern_BF`（`bgFor` 吃一串基底名），所以現在開著也不會壞。
       ⚠ 節點的 bg 是 `Northport_tavern_BF` —— 分店名接在**它**後面
         （`Northport_tavern_BF_cafe`）。交圖時照這個名字，或把節點的 bg 改成
         `Northport_tavern` 再交 `Northport_tavern_cafe_Day` 那一組。 */
    dining:{ node:'tavern', scenes:true },
    nodes: {
