...
export const TOWNS = {
  capital: {
    ...
    /* ══ Stage 1 起：蕾娜有機率出門（ver -461，Ray 定案）══════════════════
       「蕾娜在晚上六點前有機率出門，可能在行政區或教堂或戶外過道區碰到。」
       進城時擲一次（modules/town.js 的 `open()`）：中了就從 `nodes` 裡挑一格，
       六點前走到那一格會**碰到她**（立繪＋一句話）；過了 `until` 她就回房。
       ⚠ `line` 是佔位稿（Ray 只給了敲門的回話，沒給碰到時她說什麼）—— 要換直接改這一行。 */
    rennaOut:{ chance:0.5, until:18,
               nodes:['cityhall','church','midtown','oldtown','uptown'],
               line:'寫報告寫累了，出來透透氣。' },
      ...
      inn: {
        ...
        /* ══ Stage 1 起的房門（ver -461，Ray 交稿）══════════════════════════
           「夥伴進入城市就會自動亮旅店燈」——之後每次進城，夥伴預設都住在旅店裡：
             蕾娜敲門 → 「我得先寫報告，你們去吧。」（她六點前有機率出門，見 rennaOut）
