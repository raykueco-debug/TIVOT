  ...
  capital: {
    ...
    /* ══ 傍晚的提醒（ver -392，Ray 交稿）══════════════════════════════════
       「拜訪過所有地點後走到下一個場景時**或**時間抵或過 18:00：
         （現有地點，優先所有事件）諾：『天色不早了，我們先回旅店吧？』」
       ⚠ 稿子原本寫「回**廣場**旅店」，ver -393 由 Ray 改成「回旅店」——
    ...
    nodes: {
      ...
      cityhall: {
        ...
        lines:[
          ...
          /* ══ 取名那一段（ver -395，Ray 交稿）══════════════════════════════
             ⚠⚠ 稿子是接在這一句後面的（原本的收尾「啊——！」被它取代）——
               判斷依據：末句是「**晚上六點回旅店集合。明早出發。**」，那正好對上
               `TOWNS.capital.evening` 在 **18:00** 的那句提醒。在旅店（20:00）演的話
               「六點回旅店集合」已經過去了，講不通。
             ⚠ 「（那是侯爵的千金欸！你莊重一點！）」的括號是**台詞本身**（她壓低聲音講的），
               不是舞台指示 —— 所以照留。 */
          nou('surprise','好、好的，蕾娜小姐。'),
          ren('smile','真是守規矩的孩子呢。'),
          { speaker:'PLAYER', blank:true },
          ren('stare','……你倒是叫得挺順口的。'),
          /* 諾薇兒一記手刀：畫面震一下、沒有台詞（§6.5 的無台詞立繪拍，停一秒自己走）。 */
          { speaker:'NOUVELLE', text:'', auto:1000, shake:true, se:'se_punch',
            portrait:{ char:'NOUVELLE', expr:'surprise', show:true } },
          nou('whisper','（那是侯爵的千金欸！你莊重一點！）'),
          ren('smile','沒關係啦，好久沒被那樣叫，有點懷念罷了。'),
          /* ⚠ `HUND` 是她一直以來的叫法（德語「犬」），**照原文**不要翻。 */
          ren('smile','不過……我該怎麼稱呼你好呢？總不能一直叫你 HUND 吧？'),
          /* 閘門：輸入名字與暱稱（預設凱勞諾斯／凱，見 progress.js）。 */
          { nameInput:true },
          ren(null,'那麼，晚上六點回旅店集合。明早出發。'),
          nou(null,'好。'),
        ],
        /* 路人單句：**政治**線（ver -387）。 */
        chatter:[
      ...
      inn: {
        ...
        innSpots:{ sit:{ x:0.315, y:0.735 }, sleep:{ x:0.490, y:0.450 } },
        /* 初訪那一幕。⚠ 中間三拍是**演出**不是台詞：
             ① 畫面抖＋槍棺落地聲（無立繪）
