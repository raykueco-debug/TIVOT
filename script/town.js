...
const mis = N('MISHA');   // 米夏（ver -1707，走出墓門那一段起）
...
const SV_S8_DINE = { flag:'sv_s8_dine', need:'sv_s8_home', fromStage:8, lines:[
  ...
  /* ⚠⚠ **不指名菜色**（ver -1660，Ray 定案）——稿上原本是「對吧！**綠月風格的
     奶油煎鹿肉**。不用再調味就很好吃。」，而**菜色是玩家選的**（`kitchen` 那一拍
     十道任挑），沒帶食材時還會是「跟平常一樣的」。指名等於這一句有很大機率在說謊
     （十道裡有九道講錯，而畫面上看不出哪裡不對）。
     ⚠ 只拿掉中間那一句，前後照留 —— 她的語氣與「不用再調味就很好吃」都是稿上的。 */
  mar(null,'對吧！不用再調味就很好吃。'),
  sor('surprise','！！'),
  ...
  ravnsdal: {
    ...
    nodes: {
      ...
      library:  { bg:'varn_library',  name:'雪都瓦恩霍姆　圖書館',  noTime:true,
        ...
        acts:[ { flag:'vn_lib_done', need:'vn_evening', goto:'midtown',
          ...
          Object.assign(sor('read','『10月12日14時，評測對象展現高超戰鬥技術，適性初步判斷：良好。』'),
                        { se:'se_ui_pageflip' }),
          Object.assign(sor('read','『10月13日16時，與隊友連攜狀況良好，適性判斷維持。』'),
                        { se:'se_ui_pageflip' }),
          nou('nod','嘿嘿……'),
          ...
          sor('worry',''),
          ren('shout','我的評鑑報告！'),
          { speaker:'NARRATION', text:'', se:'se_snatch', auto:1200 },
      ...
      /* ⚠⚠ 站房上那面大鐘的指針**由遊戲時間擺**（ver -1249，Ray：「不然背景的時間
         ...
         ══⚠⚠⚠ ver -1541：**圖換過了，整組重量**（Ray：「瓦恩霍姆的火車站圖有改，
           時鐘的位置角度要修正」）══
         舊的那一組（錶心 (767,170)、r 50px、wipe 100.5°）是對著**上一張**站房量的，
         換圖之後那塊補丁落在石牆上，而真正的錶盤原封不動 —— 畫面上就是
         「牆上多一塊灰圓，鐘還是永遠停在同一個時間」。

         **現在這一組（`Varn_Station_*.webp`，1536×1024，逐張量過）：**
           · 錶心 (904, 255) ⇒ (0.58854, 0.24902)
           · 盤面半徑 44px ÷ 1536 ＝ 0.02865（奶油色盤面的外緣，外面是深色石框）
           · 畫上去的兩根指針：**48°** 與 **324°**，都由錶心伸到 r≈0.80

         ⚠⚠⚠ **這張錶盤不能整面蓋掉** —— 它有一圈羅馬數字（r≈0.65~1.0），
           所以 `faceR:0.22`（只補錶心那一塊）＋ 兩條 `wipe` 把指針各自抹掉。
           `face`／`hand` **不寫**：顏色由引擎量現在畫面上那一張（四個時段差很多，
      ...
      station:  { bg:'varn_station',  name:'雪都瓦恩霍姆　火車站',  
        ...
        clock:{ x:0.58854, y:0.24902, r:0.02865, faceR:0.22, hLen:0.60, mLen:0.86,
                wipe:[[48, 0, 0.86, 0.08], [324, 0, 0.86, 0.08]] },
        exits:{ back:'oldtown' },
      ...
      inn:      { bg:'varn_hotel',    name:'雪都瓦恩霍姆　旅店',
        ...
        /* ══⚠⚠⚠ **劇情合流：古墓之後、隔日早上 08:00**（ver -1707，Ray 交稿「[非線性Stage 10 A route結束]」）══
           ...
           ⚠⚠ **沒有「分支 1 × M2」（AB・M2）**（ver -1714，Ray：「應該沒有 ABM2」「把那個死版本拿掉」）：
             ...
             （原 `script/branches.js` 的 `ROUTES`，-1734 隨分歧鈕回收）。分支 1 那個 act 因此不帶 M 的條件。
        ...
        { flag:'vn_after_tomb', stage:14, need:['tomb_exit_done','ep_belisar_done','ep_m2_route'], sides:{ RENNA:'L' }, lines:[
          ...
          sor('cry','因為……因為……'),
          sor('crybig','妳們都會回聖王廳，就剩我一個了啊！'),
          nou('awkward','我會去看妳啦，夏爾村那麼近。'),
          ren('smile','是啊，坐火車半天就到囉。雖然還要走一段。'),
          sor('cry','我會去接妳們啦！'),
          sor('cry','你也要來喔！'),
          { speaker:'PLAYER', blank:true },
          sor('crybig','瑪麗亞的料理比我重要嗎！'),
          nou('bigsmile','他害羞啦。'),
          sor('cry','嗚——小公主也要來啊！'),
          any('talk','我……'),
          any('silent','我就留在這裡就好了……'),
          nou('surprise','安雅……'),
          sor('surprise','為什麼！'),
          any('silent','……'),
          ren('stare','……那可不行。'),
  ...
  eastport: {
    ...
    nodes: {
      ...
      inn:        { bg:'east_hotel',      name:'東方泊地　旅店',
        ...
        /* ══⚠⚠⚠ **走出旅店**（ver -1532，Ray 交稿）══════════════════════════════
           > Ray：「走出旅店：[分支開始] 分支1：還沒完成古墓探索劇情…
           >        分支2：已完成古墓探索劇情… [分支結束]」

           ⚠⚠ 接在**審訊**（`ep_interrogate`）之後 —— 那是 Ray 回報的
             「M1 敲淑女門很失禮以後走出旅店為什麼沒接上劇情？」：那一支旗
             在此之前**沒有任何人讀**，10-B 演完就斷在旅店裡。
           ⚠ 用 `onLeave` 不是 `acts`：語意就是「你要走了」（同北泊 `np_anya_join`、
             夏爾村 `shinier_siege`）。**出航也吃得到** —— 那一樣是離開這一格。
           ...
           ⚠ 分支 1 的 `until:'tomb_done'`：先跑完古墓再回東泊的人（先 A 後 B）
             直接看分支 2，不會補演一段「下一個是伊甸古墓」。
           ⚠ 四人同台分兩邊：只覆寫 `RENNA:'L'`（§6.5 的站位表，碰到安雅蕾娜放左）
             ⇒ 左：蕾娜・諾薇兒／右：索菈娜・安雅。
           ⚠⚠ **稿上三處說話者與立繪對不上，照立繪判**（同 -1346／-1523 的作法）：
        ...
        /* ══⚠⚠⚠ **那一夜：奪回髮飾**（ver -1388，Ray 交稿）══════════════════════
           ⚠⚠⚠ **好感的門已經拿掉了**（ver -1489，Ray：「把髮飾事件的好感觸發鎖
             ...
             「若蕾娜 T2 隔日正常探索／若蕾娜 T3 …」分兩條。
             現在**沒有 T2 那一條** —— 只要 `ep_bel_back` 演完，任何好感都會跑這一段。
           ⚠⚠ 為什麼是拿掉而不是把門檻降到 1：**門檻 1 仍然是一道門**，
             而「必跑」的意思是這一段不再由好感決定（鐵律 9：旗與門要答得出
             「誰擋的」—— 答案是「沒有人擋」就不該留一個永遠成立的條件在那裡，
             下一個人會以為它還有作用）。
           ⚠ `needTier` 這個機制**留著**（`modules/town.js` 的 `actDue`）：它是通用的
             一格，只是目前**沒有任何一段在用**。
        ...
        /* ══⚠⚠⚠ **改由「回房睡覺」觸發**（ver -1396，Ray：「強制回到東泊時的睡覺
           ...
           ⚠⚠ **小睡跟著這一段走**（ver -1489 起沒有好感的門，所以是「每一次」）：
             `ep_bel_back` 演完之後的第一次按睡覺一定是那一小時的小睡 ——
             這一段自己的 `flag` 演完就記，`actDue` 下一次就跳過它，
             之後按睡覺就是正常睡到隔天。
             ⚠ 舊版這裡是「`needTier` 連小睡一起管」（T2 那一晚睡得著、直接到隔天）
               —— 那一條分歧已經不存在了。
           ⚠ 它**不需要**另一支旗來收尾：這一段自己的 `flag`（`ep_night_raid`）
             演完就記，`actDue` 下一次就跳過它 ⇒ 之後按睡覺就是正常睡到隔天。 */
        {  flag:'ep_night_raid', need:'ep_bel_back',
                 sleepFirst:{ hours:1 },
    ...
    nodes: {
      ...
      inlet:      { bg:'lake_inlet',      name:'鏡湖　山口',
        ...
        /* ══⚠⚠⚠ **出航**（ver -1532，Ray 回報「鏡湖怎麼沒有出航口？」）══════════
           ...
           （`inlet`，**出航那一格**）」，但這一格從來沒有 `sail`
           ⇒ **降落到鏡湖之後整張圖出不去**（只有 `up` 一個方向，走到底是死路）。
           ⚠⚠ **不掛 `flag`**：同伊甸古墓／石製遺蹟 —— **只飛得到的地方，到得了
             就走得了**（ver -1154，Ray 回報過一模一樣的「進了遺蹟無法出航」）。
             要旗的是「有陸路可以走到」的那一族（帝都／夏爾村），那裡「還沒有船」
             才是真的成立。
           ⚠ 自檢（-1532 跑過一次）：把 `flight/index.html` 的 `PLACES` 裡所有
             `town:'…'`（降得下去的 12 張圖）對一次 `script/town.js`，
             **每一張都要有而且只有一個 `sail`**。當時只有 `lake` 是 0。 */
        sail:{},
...
*/
  ...
  tomb: {
    ...
    wilderness: true,     // 野外的路沒有門可以關（19:00 全域打烊不罩，ver -862）
    stepMin: 10,          // 遺蹟那一級（ver -917，Ray：「遺蹟內每次移動 10 分鐘」）
    /* ══⚠⚠⚠ **追逐機制的參數**（ver -1576，Ray 逐項定案；runtime 見 ver -1577）══
       資料先立在這裡（鐵律 1），程式接上時只讀它、不要把數字寫進 modules。
       ⚠ 讀它的是 `modules/town.js` 的 `chaseStep`／`chaseActDue`／`chaseAfterAct`
         那一族（ver -1577 接上，唯一的實作，鐵律 8）；狀態存在 `progress` 的
         `tivot_chase_v1`（一輪內，跟著存檔走）。
         完整說明（含逐項算出來的場數）在 `HANDOFF.md` 的「伊甸古墓・追逐機制」。
       ⚠⚠ **`wildRate`／`firstWildAt` 還沒有人讀** —— 那是雜兵那一半，古墓 28 隻的
         卡還沒到（`resources/enemy/_tomb_mon_spec.md`）。而且它們與 `wildSpawn.rate`
         是同一件事的兩個真相：卡到齊時**要先決定留哪一份**，不要兩邊都寫（鐵律 7）。

       · `startFights` ＝追兵**何時上線**：**打過 3 場之後的下一格**（ver -1593，Ray：
         「把規則改成三戰後下一格才出守護者劇情」）。
         ⚠⚠ **-1576 的「踩到第 4 格」那一條拿掉了**（`startStep`）：它與場數是
           「先到者」，而雜兵在 -1591 之前根本刷不出來 ⇒ 實際永遠是「走四格就撞上
           守墓者」，第一格就見到牠。Ray 回報的就是這個。
         ⚠ 「下一格」是字面的：第 3 場打完之後**再走一步**牠才出現。
       · `startStep` **已停用**（留著欄位當紀錄，沒有人讀）。
       · `gap` ＝**沒有用到了**（ver -1593）：現在是「三戰後下一格**就出現**」，
         牠直接站在你腳下那一格 —— 那一場就是守墓者的登場戲。欄位留著當紀錄。
       · `speed` ＝玩家每動一格牠推進幾格（擊退、停完 `stun` 之後才開始算）。
       · `onEncounter` ＝遭遇雜怪時牠**再**推進幾格（玩家停下來打了一場）。
       · `stun` ＝被追上、打贏之後牠停幾回合。
       · `resetAt` ＝這一格必觸戰鬥，戰後**牠的位置設成這一格**再停 `stun`
         （Ray：「從柱廳戰後從柱廳開始停 然後追」）。
       · `idleAt` ＝這幾格牠不推進（墓門）。
       · ⚠⚠ **安全點不重置牠的位置**（Ray 明講）—— 所以沒有 `resetAtRest` 這種欄位：
         安全點只結算＋回血。回頭走的話牠可能已經很近了，那正是這張圖的壓力來源。
       · ⚠⚠ **追兵與雜怪競合 ⇒ 追兵優先**（Ray 明講）。
       · ⚠⚠⚠ **打輸就是既有的那一頁**（ver -1577，Ray：「不用，就是**聖光黯滅**
         那一套」）—— **不新增 lose kind**。-1576 那一版寫的「要新開一種」已作廢，
         `loseKind:'gameover'` 這一格也拿掉了（沒有人讀的欄位就是下一個謊）。
         ⚠ 古墓沒有旅店、這幾場又是插入戰 ⇒ `main.js` 的 `setLoseKind` 現行分流
           本來就給 `rollback`（繼續＝回檔／放棄＝主畫面），不必為它動任何程式。
    ...
    /* ══⚠⚠ **遇敵率 50%**（ver -1614，Ray：「改成 50% 吧，遇敵率實在太低，
       ...
       ⚠ 為什麼要這麼高：墓門→柱廳最短 **10 格**，扣掉沿路 `noWild` 的那幾格
         大約 9 次擲骰 —— 33% 的期望值剛好是 3 場，於是「三戰才出追兵」有
         四成機率在抵達柱廳前湊不滿（Ray 實測到的就是這個）。50% 之下
         擲不滿 3 場的機率降到約 9%。
    ...
    /* ══ 槍棺地圖（ver -1145）══════════════════════════════════════════════
       ...
       ⚠ 這張**有迷霧**（上面那一條）：沒走到的格子蓋一團霧，所以墨點準不準
         玩家是一格一格看出來的 —— 走錯一格就露餡。 */
    /* ══⚠⚠⚠ **小地圖：一層一張，共三張**（ver -1649，Ray：「一層一張，共三層」）══
       ⚠⚠ `sheets` 是**由上往下取第一個「裝得下你現在那一格」的那一張**
         （`modules/town.js` 的 `mapSheet()`，鐵律 7/8：挑哪一張只有那一支在算）。
       ⚠⚠⚠ **圖與墨點是同一次算出來的**（`tools/map_compose.py`，§6.5.4.4 的 -1145）
         —— 座標照抄 `resources/map/_spots_tomb_l?.json`，**一個數字都不要用眼睛估**。
         改動拓樸之後要重跑那一支，兩邊一起換。
       ⚠ 舊寫法（單張 `img`＋`spots`）照舊吃得到 —— 其他 12 座城還是單張。 */
    map: {
      sheets: [
        /* ── 一層（21 格）── */
        { img:'resources/map/map_tomb_l1.webp', spots:{
          ...
          crossing:[0.4551, 0.4498], chantry:[0.2702, 0.4498], scriptorium:[0.0853, 0.4498],
          chapel:[0.6400, 0.4498], confessio:[0.8249, 0.4498], ambulatory:[0.6400, 0.2947],
          sacristy:[0.6400, 0.1396], apse:[0.4551, 0.2947], reliquary:[0.4551, 0.1396],
        ...
        { img:'resources/map/map_tomb_l2.webp', spots:{
          ...
          censerroom:[0.3626, 0.4304], sarcE:[0.4551, 0.3335], ossuary2:[0.5475, 0.3335],
          kiln:[0.5475, 0.2366], nichehall:[0.3626, 0.2366], ossuary:[0.2702, 0.2366],
          shaft:[0.4551, 0.4304], urnvault:[0.6400, 0.3335], ashpit:[0.6400, 0.4304],
          graveldrop:[0.5475, 0.4304], saltroom:[0.6400, 0.2366], chimney:[0.6400, 0.1396],
          nitrepit:[0.7324, 0.2366], masonhall:[0.7324, 0.3335], toolniche:[0.8249, 0.3335],
          rubble:[0.7324, 0.4304], cryptwalk:[0.6400, 0.5273], deepwalk:[0.6400, 0.6243],
          brokenhall:[0.5475, 0.6243], sunkhall:[0.6400, 0.8181], mudpit:[0.6400, 0.9150],
    ...
    nodes: {
      ...
      /* ⚠⚠ **墓門：不出怪、追兵也不推進，但它不是安全點**（ver -1576，Ray：
         「墓門不是安全區但是也不出怪也不會追」）——
         `noWild:true` 管前者；**沒有 `rest:true`** ＝走回來不結算、不回血。
         ⚠ 這個分別是刻意的：它是「還沒開始」的緩衝，不是「可以回來喘息」的地方。
           真的能喘息的只有三個安全點（二層梯廳／三層梯廳／底層祭壇）。
         ⚠ 追兵在這一格不推進 —— 那一條由 `chase.idleAt` 管（見城上的 `chase`）。 */
      gate:       { bg:'tomb_gate', noWild:true, name:'伊甸古墓　墓門',
        ...
        acts:[ { flag:'tomb_gate', fromStage:8, sides:{ RENNA:'L' }, lines:[
          ...
          /* ⚠⚠⚠ **ver -1521：城名由「拉芬斯達爾城」改成「瓦恩霍姆城」** ——
             -1191 定案的舊名，**-1491 已經整座城改名成「雪都瓦恩霍姆」**
             （`TOWNS.ravnsdal.name`），而這一句是全稿唯一提到城名的地方、
             **沒有人跟著改** ⇒ 演出來會是一個遊戲裡根本不存在的地名。
             ⚠ 這正是 -1491 那一輪自己的教訓（`SETTLE_RENAMES`）的另一半：
               那張表接得住**存檔裡的鑰匙**，接不住**台詞裡的字**。
               **改 `TOWNS[].name` 之前，順手 grep 一次舊名。** */
          ren(null,'先到瓦恩霍姆城休整一下吧，或許能有相關的情報吧。'),
          /* ⚠⚠ **ver -1521 新增**（Ray 的新稿：「分支派生，若還未去過貝利薩爾」）——
             與開頭那句「為什麼偏偏先來這裡……」同一個條件的另一半：
             **先來這裡的人**才聽得到她提議「不然先去貝利薩爾」。
             ⚠ 底下索菈娜／諾薇兒那兩句**不掛條件**：-1189 Ray 親口確認過
               「多一句是指那一句」，而那兩句在兩條路上都讀得通（她就是想逃離這裡）。
               ⚠ 新稿的排版把它們排在同一個「分支派生」底下，**讀得成兩種**；
                 沿用 -1189 已經確認過的那一種，要改說一聲。 */
          ren('smile','不然，我們就先去貝利薩爾遺址。好不好？', { skipIf:'belisar_seen' }),
          sor('whisper','我說……修女小姐是不是全力想逃離這裡？'),
          nou('awkward','好像是……'),
          /* ⚠⚠ **M1 路線派生**（Ray 的新稿：「M1路線派生」）——
             安雅敢說「交給我」是因為那一夜她被蕾娜跟上、身分說破了。
             ⚠⚠⚠ **ver -1531：判準由 `onlyIf:'ep_m1_route'` 改成
               `skipIf:'ep_m2_route'`**（Ray：「**先 A 走 M1 聽得到啊**」）——
               與雪都那三處同一條原則（ver -1530）：A route（古墓）與 B route
               （東泊）是**非線性**的，先跑古墓的人兩支旗都沒有，而**那個狀態
               算 M1**（＝氣氛和諧、沒有秘密的那一條）。
               · -1521 寫成 `onlyIf` ⇒ 先 A 的人聽不到這一句。那不是「少一句」，
                 是把先 A 默默歸給了 M2 那一側。
               · 兩支旗互斥是資料保證的（M2 的 `until` 就是 M1 的 `need`），
                 所以 `skipIf:'ep_m2_route'` ＝「M1 或還沒發生」。
               ⚠ **全專案判 M1／M2 一律用 `ep_m2_route` 當判準**（鐵律 7：
                 一個分叉一個判準）—— 用 `ep_m1_route` 判就會把先 A 丟到錯的那一邊。
             ⚠ 上面那兩句（「幽靈……要是有就好了。」與蕾娜的「連安雅小姐都……」）
               **不受這一條管** —— 稿上它們在分支之外。 */
          any('answer','幽靈……交給我。不要怕。', { skipIf:'ep_m2_route' }),
          /* ⚠ 稿上寫的 `dyingcute` 沒有這張圖，ver -1189 由 Ray 改指
             `Renna_SI_scarecute`（鍵 `scarecute`，與 `cutescare` 同一張）。 */
          ren('scarecute','啊——夠了！出發了！'),
        ] },
        /* ══⚠⚠⚠ **墓門・stage7 以前**（ver -1521，Ray 交稿：「伊甸古墓stage7以前」）══
           還沒接到任務就自己飛過來看的那一趟：**只有蕾娜與諾薇兒講話**
           （索菈娜與安雅那時還不一定在船上），六句就走。
           ⚠⚠ 與上面那一段是**靠章節互斥**的（`untilStage:8` vs `fromStage:8`），
             所以**順序不影響**誰先取到 —— 排在後面只是因為上面那一段先寫。
             ⚠ 兩段各有各的 `flag`：stage7 以前看過的人，到了 stage8 照樣演長的那一段
               （那是兩件事，不是同一段的兩個版本）。
           ⚠ 這一段**不寫 `sides`**：台上只有蕾娜（本位右）與諾薇兒（本位左），
             本來就分兩邊（§6.5 的站位表）—— 安雅不在場，蕾娜不必讓位。
           ⚠ 諾薇兒那兩句稿上沒標表情 ⇒ `expr` 給 null（不動立繪，同上一段的作法）；
             她這一段的第一次上場就是底圖那一張。 */
        { flag:'tomb_gate_early', untilStage:8, lines:[
          ren('cringe','為什麼會飛到這種地方來？'),
          nou(null,'這是……什麼地方？'),
          ren('talkwork','北境的伊甸古墓。'),
        ...
        /* ══⚠⚠⚠ **走出古墓回到墓門**（ver -1707，Ray 交稿）══════════════════════════
           ...
           判準照雪都那幾處的慣例：**`ep_m2_route` 插著才是 M2**，兩支都沒有的（先跑古墓）走 M1。
           ⚠ `need:'tomb_altar_done'` ＝祭壇終戰那一段演完（廢坑道開了、人從那裡走回來）。
        ...
        { flag:'tomb_exit_done', need:['tomb_altar_done','ep_m2_route'], goto:'@ravnsdal:inn',
          ...
          Object.assign(any('desperate',''), { cg:null, amb:null }),   // 插圖結束（ver -1732）＋ troop 停（-1733）
          ...
          sor('back','好像是。年紀看起來差不多。'),
          ren('coldstare','……'),
          ren('think','安雅……安娜……米夏跟……米海爾？'),
          ren('lookaway','雙胞胎……？'),
          { speaker:'PLAYER', blank:true },
          ren('shockcalm','！！'),
          ren('shout','為什麼沒有第一時間告訴我！'),
          sor('cringe','呃——'),
          ren('callangry','米海爾殿下！'),
          mis('side','！！'),
          any('terrify','！！'),
          nou('faint','？？'),
          ren('callangry','您是謝索洛夫皇國的第一皇子，米海爾殿下沒錯吧？'),
          mis('frown','……'),
          ren('salute','我是聖王廳第十三騎士團一等監察官，蕾姬娜˙海森伯格。'),
          ren('invite','基於永夜協議，我代表聖王廳要求說明，您所謂安娜殿下的使命是？'),
          mis('frownopen','一等監察官……看來你也懂我們的語言。'),
          mis('closeopen','安娜，竟然連身份都告訴他們了嗎？'),
          ...
          ren('coldstare','只是……'),
          ren('upset',''),
          /* 蕾娜轉身離開：高跟鞋聲 ＋ 好感降到這一級的最低值（稿，`affToFloor`）。 */
          { speaker:'NARRATION', text:'', se:'se_highheels', hide:['RENNA'], affToFloor:'RENNA', auto:1400 },
          nou('faint','啊！蕾娜小姐！'),
          sor('tire','唉，好不容易才和好，換你搞砸了啊。'),
      ...
      vestibule:  { bg:'tomb_vestibule', name:'伊甸古墓　前庭', noTime:true, noWildFirst:true,
        ...
        /* ══⚠⚠⚠ **墓主遭遇之後回到門廳：門已經關上了**（ver -1635／**-1636 由墓門搬到前庭**，
           Ray：「門廳是前庭」）══
        ...
        { flag:'tomb_sealed_in', need:'tomb_chase_on', sides:{ RENNA:'L' }, lines:[
      ...
      hall2:      { bg:'tomb_hall2', name:'伊甸古墓　柱廳', noTime:true,
        ...
        /* ══⚠⚠ **追兵帶著走的三段戲**（`chase.scenes`，ver -1616）══════════════════
           三段都標 `chaseOnly:true` ＝**只有被追上帶得動，走進柱廳不算**
        ...
        /* ══⚠⚠⚠ **小隊分組 —— 這一段只在柱廳演**（ver -1607，Ray：「什麼跟什麼啊
           ...
           的劇情接上去」）══════════════════════════════════════════════════════
           追逐的登場戲（上一段）到「什麼跟什麼啊沒完沒了！」**就結束**；
           分組（含 Execute 插圖、`tomb_chase_on`／`tomb_split`）是**踩到柱廳**才演的。
           ⚠⚠ -1599／-1603／-1606 我連錯三次，每次都把這一段一起拖到追兵現身的
             那一格去接 —— **它不在 `chase.intro` 上**（那一支指名的是 `tomb_gk1_done`）。
        ...
        { flag:'tomb_gk1_split', need:'tomb_hall2_arrive', sides:{ RENNA:'L' }, lines:[
          ...
          /* ⚠⚠⚠ **`tomb_chase_on` 已移到登場戲的收尾**（ver -1608，見那一段）——
             這一拍只剩 `tomb_split`（＝分組了，搭檔只剩諾薇兒）。
             ⚠ 兩支旗本來就各管一件事（鐵律 9），-1607 把整段分組切成柱廳專屬之後，
               `tomb_chase_on` 跟著被關進來 ⇒ 先被追上的人永遠插不到它
               ⇒ **背安雅（`tomb_carry`，`need:'tomb_chase_on'`）整段不見**
               （Ray 回報）。 */
          Object.assign(sor('ready','一會見！'), { flags:['tomb_split'] }),
          /* ══ 好感分歧（諾薇兒）══
      ...
      sarcE:      { bg:'tomb_sarce', name:'伊甸古墓　石棺室', noTime:true,
        exits:{ left:'rotunda', right:'ossuary2', down:'shaft' } },
      ossuary2:   { bg:'tomb_ossuary2', name:'伊甸古墓　甕棺廊', noTime:true,
        exits:{ left:'sarcE', up:'kiln', right:'urnvault' } },
      /* ← 死胡同 G（三格深） */
      ...
      lowaltar:   { bg:'tomb_lowaltar',
        ...
        acts:[ { flag:'tomb_altar_done', need:'tomb_low_arrive', storyBattle:true, sides:{ RENNA:'L' }, lines:[
          ...
          /* ⚠⚠ **`tomb_h_route` ＝走了 H 路線**（ver -1707，稿：「T3 以上[H路線]差分結束 插旗[H路線]」）——
             插在 T3 分支的最後一拍（這一拍本來就只有 T3 以上演得到）。誰插＝這一拍、誰拔＝沒有人。
             走出墓門那一段的 H 派生與「H／非 H 換立繪」全部讀它（`onlyIf`／`skipIf`）。 */
          ren('hugshy',  '……嗯。謝謝你。',                  { tierMin:3, tierWho:'RENNA', flags:['tomb_h_route'] }),
