    ...
    story: {
      steps: {
        ...
        /* ⚠⚠ **空陣列＝這一段劇情版不演**（ver -938，Ray：「這一段就這樣而已」）——
           ...
           （「小心！攻擊要來了！」「牠蓄力的時候會出現光圈」）與下面 `threat`
           那一句講的是同一件事，光圈都還沒出現就先講兩句，讀起來是拖。
           ⚠ **不可以把這個鍵刪掉**：`linesForStep` 查不到就會**退回芙蕾雅那一份**，
             於是諾薇兒的教學裡會冒出監察官的台詞。要「不演」就明寫 `[]`。 */
        board1: [],
        /* ══⚠⚠ **先叫他點掉，規則留到點完再講**（ver -726，Ray：「開頭的諾薇兒
           ...
           ⚠ 所以這裡只有一句，而且不解釋任何規則。 */
        /* ⚠ 這一拍同時亮起**指著光圈的雪鐵龍箭**（閘門寫在 config.tutorial.steps 的
           threat 那一段，ver -938）——玩家要**點掉那顆光圈**才過關，點畫面沒有用。
           ⚠ 文案 ver -938 由 Ray 定稿，照抄不要改寫。 */
        threat: [
          { who:'nouvelle', img:'tut_nouvelle_surprise',
            text:'敵人的攻擊要來了！用強力的副武器阻止他！' },
        ],
        defended: [
          /* ← 「越接近越痛」要等他**已經點過一次**才聽得懂 —— 位置就是 Ray 指定的
             「點完以後」（-726 立、-938 換文案）。⚠ 照抄不要改寫。 */
          { who:'nouvelle', img:'tut_nouvelle_surprise',
            text:'越接近敵人攻擊的瞬間反擊威力越強！' },
