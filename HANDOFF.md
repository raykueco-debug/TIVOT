...
## -854 全流程監測掃查（2026-09-06，Ray：「去測城鎮、戰鬥、飛行…有否疊加、
該殺沒殺的狀況，把故事跑一次到最新進度然後監測」）

用探針 2.0 的 __snap 逐態抓（S0 帝都→S1 出航船戰→S3 北泊城鎮戰×2→S6 夏爾村
sv_wild 頭目戰＋評價亂入）。**逐態結果**：

| 態 | painted ∞動畫 | ♪ | timer | rAF/s | 判定 |
|---|---|---|---|---|---|
| 首頁閒置 | 3（全 opacity） | 1 | 5 | 60(HUD) | ✔ |
| 劇情/城鎮 | 3~7（槍棺配件+羅盤箭，全 transform/opacity） | 1 | 5 | 60 | ✔ |
| 店舖態 | 4 | 1 | 5 | 60 | ✔ |
| 飛行巡航（主頁視角） | **0** | **0**（主頁 BGM 已收） | 5 | 60 | ✔ |
| 戰鬥中 | 0~3（gridGlowPulse/moonCharge 全新版 opacity） | 1 | 7 | 120(HUD+延時圈) | ✔ |
| 打完回城（中間場） | 回到城鎮組 | 1 | 5 | 60 | ✔ 零殘留 |
| 回首頁（各路） | 3 | 1 | 5 | 60 | ✔ killAllPages 乾淨 |

- **戰鬥↔城鎮循環兩輪零漂移**（heap 恆 5MB、dom 打完回穩、timer 5↔7 對稱）。
- **飛行→戰鬥交棒**：iframe 確實被殺（heap 60→5MB 回收）；**敗北→繼續→死 frame
  復活**（-846 路）實測 OK，船在原座標。
- **索拉娜搭檔 BGM**：sv 戰鬥中單一曲、duration 161.1s＝Peritune_Whirlwind ✔ 無疊播。
- **評價＋亂入**：sv_wild 打完 D 級、RennaSorana 亂入差分與台詞正常；縮半圖全程無異狀。
- **抓到並修掉（ver -854）**：打完回飛行（keepPages 路）戰鬥層留著壓在 iframe 底下
  照畫（moonCharge/moonBeam 巡航全程偷跑）→ 補 `body.flight-on #top/#bottom` 藏＋停。
- **已知殘項（小）**：結算/敗北頁是半透明的，月扣動畫在底下照畫（現在是 opacity 版，
  成本低；要收就在 over 時拔 .charging）。
- **測試假象（不是 bug，兩個 session 都撞過）**：跳過 act 直接 `startScriptBattle`
  會讓城鎮舞台不收、戰鬥在底下隱形跑，結算後落在首頁且 BGM 不接回 —— 真實玩家
  路徑（act 的 `{battle:…}`）不會走到；synthetic 測試要從 act 觸發才準。

## -853 探針 2.0：常駐開關鈕＋全面指標（2026-09-06，Ray：「把所有可能吃效能、
重疊吃效能、播放中的東西都放進去」）

- **右下常駐開關鈕 `#perfToggle`**（管理人模式限定，CSS 白名單樣式；z 99999 ＞
  飛行 iframe，任何畫面都按得到）—— 進遊戲後也能開關 HUD；團徽連點 5 下照舊。
- **HUD 新指標**（計數層 index.html ＋ 顯示層 main.js debugHud）：
  · fps ＋ **最長幀 ms**（卡頓尖峰）｜long task 5s 累計
  · dom 數＋**開機以來增量**（越玩越多＝洩漏）
  · **圖估 MB/張數**（<img> 寬×高×4、同 URL 去重 —— 縮圖實驗的量尺）＋ canvas MB
  · **rAF /s**（>70 ＝不只一條 rAF 迴圈）｜**sto /s**（setTimeout 呼叫速率）｜
    **網5s**（5 秒內請求數/KB —— 載入迴圈、404 重試在這行現形）
  · 層：home/stage/**戰鬥**（改讀 #top，#home 長在 #app 裡整層永遠 visible）/flight
    ＋ **疊:**（prep/gear/loot/menu 誰開著）
  · **動畫中 N（∞ M）**：正在畫的動畫總數＋無限循環清單（含 ::after）
  · **♪ 播 x/載 y**｜src♪｜**ctx:running/suspended**（驗 -852 背景守門）
- **探針立第一功**：首頁有一條 60fps 常駐 rAF —— `combat.ringTick`（延時倒數圈）
  由 bootIdle 帶起、沒人收。修法：ringTick 開頭 `state.over` 就自滅（鐵律 10），
  下一場 startIntervalTimer 重新拉起。實測：首頁 0 條（HUD 關）、戰鬥中 1 條、
  打完回首頁自滅。
- ⚠ 探針只看得到**主頁 document**：飛行頁內部（它自己的 audio/rAF）要看飛行頁
  自己的 HUD（?debug 那套）。

## -852 背景燒電：切背景＝音訊全停（2026-09-06）

Ray 回報「關掉網頁許久後仍發燙」→ 查電池：web App **背景活動 16 分鐘**（螢幕 1h26m）。
不是 RAM 洩漏（佔記憶體不發熱、關掉程序就死）——是**音訊把程序在背景保活**：
分頁凍結時 rAF／計時器停，但正在播的 `<audio>` 照播（§6.10 -388 早記過這行為，
當時只在「進戰鬥」收，**鎖屏／切 App** 這條路沒人收）。

- `audio.js`：`visibilitychange` → hidden＝BGM pause＋ctx.suspend；visible＝resume＋
  曲子還是現任才接回（音量直接寫回 `bgmTargetVol()`，背景期間可能凍在淡入淡出半路）。
  這也堵「解鎖後 iOS 自己恢復舊曲與新曲疊播」那一族的雙 BGM（Ray 之前回報過、桌機
  重現不了的那件）。
- `flight/index.html`：同一條規矩走**既有的** `flightAudio(false/true)`（鐵律 8，
  與 `__flightResume` 同構）；visible 要 `!extPaused` 才開回來。FLIGHT_VER → -852。
- 桌機實測：hidden 當下在播媒體數 1→**0**，visible 後同一首以目標音量接回。
- 判讀基準：這版部署後，電池那格「背景活動」應趨近 0；不歸零再查別的保活源。
...
- ⚠ 代價：打完回飛行會**重新跑一次飛行讀取頁**（-388「不重載」的設計被此指令推翻）。
  **Ray 拍板：「正常的，所以飛行戰才沒有設計得那麼密集」**——遭遇頻率本來就低，
  這個讀取成本是接受的設計；不要為了省它把 iframe 改回常駐。⚠ 首勝對白（scripted 路）只驗了程式路徑（共用 afterBattleReturn），
  沒真打一場劇本遭遇 —— 下次跑主線時順帶看一眼。
- 其餘發燙候選（未動）：戰鬥常駐的 CSS 動畫（saintPulse/alert 的 drop-shadow）、
  開機預載總量 —— 若 Ray 還嫌熱再逐項量。

## -848/-849：鐵律 10「一次只跑一個 frame」（Ray 入憲）＋發燙第二輪

- **Ray 入憲（CLAUDE.md 鐵律 10）**：「應該一次都只跑一個 frame…不使用的 frame 都
  殺掉，要用的時候再重新讀。每一個 frame 的東西都不多，讀取很快，把它全部壓在
  下面拖效能更蠢」。新增全螢幕層要先答「誰蓋住我時我怎麼死／回來怎麼重生」。
- **-848 抓到的元凶**：town-story 畫面下 9 條無限動畫在**被蓋住的層**照畫 ——
  戰後 goHome(keepPages) 把 #home 掛回 .on、舞台蓋上後首頁的 filter 動畫
  （emblemGlow/pulse＝逐幀重繪）在底下燒到天荒地老；#app 的月光計量表同理。
- **落地**：①#home 離場即拔 .on（goHome 過場 600ms 檢查窗：新頁開了才拔，
  沒開留著當 -585 那族的保底）②`body:has(#storyStage.on) #app{visibility:hidden}`
  （storyStage 是 body 直屬，藏 #app 不誤傷）＋動畫暫停保險③首頁 emblemGlow/pulse
  由 filter 呼吸改 **opacity 呼吸＋靜態光**（合成器動畫）。修後同畫面無限動畫 9→4。
- 實測：城鎮/劇情全程正常；開棺交棒 stage-off 與 app-visible **同一拍**（原子切換）。
- ⚠ **BGM 疊播（Ray 回報「類型畫面的BGM有兩首一起在播」）桌機重現不了**：
  整條村戰流程側錄只有一顆播放器。已回問 Ray 是哪個畫面/哪兩首/是否 -847 後才出現
  （若是 iOS 鎖屏回來的中斷恢復，修法不同）—— 等回覆。
