...
· 安雅的非夢魘化夢境破碎＝**固定 5 秒**（主動技卡上的 `buffSec:5`，不吃赤足／鐵蹄的延長）——
  「用來跟 Boss 丟的密集圈對沖的技能，不用太長駐，免得又變永動機」。
· **三女主平衡定案，不再改的**（試算表分析頁已照這個寫）：
  · 索菈娜 ②箭頭＋⑤海宣 4 份（約 Lv5）就成循環 —— **刻意的**：她最晚加入、等級最低，紅利要往前放，不然會早早被棄用。
  · 諾薇兒中期不算弱：②引路對真人玩家很有利，②＋⑥斷鉗基本就是 MB 保證。
· 沒實打。

# （上一段）截至 `ver 2026.09.22-1779`（-1779：cut-in 一律凍結、SI／NI 上滑拿掉聚光）

**`-1779`**（Ray：「要播 cut-in，但 cut-in 期間遊戲要凍結，不可在 cut-in 期間還在縮圈」「cut-in 規則應該通用，只要 cut-in 就 freeze」
「NI 狀況下上滑不要再有聚光效果，SI 也一樣，滑就是滑，不然我常常滑不出夢碎」）
· `combat.cutinFreeze／cutinThaw`：縮圈（`defense.pauseThreats`）與延時倒數停在當下，撤下原樣接回；
  掛在 `saint.playCutin` 與 `playSaintCutin` 這兩個 CI 唯一入口。疊層安全（對話層已凍著就不重複解）。
  ⚠ 既有幾張 CI 撤下後的 `resetEnemyTimers`（清圈重排）**沒動** —— 凍結是新規則，清圈是那幾招各自的慣例；要一起拿掉再說。
· 非夢魘化的夢境破碎（-1778）改成**要演 cut-in**（凍結），撤下才起算增益，**不清圈**。
· SI／NI 上滑層（`#returnSwipe`）：聚光不再出現；上滑門檻放寬（距離 ×0.8、斜度 0.8）。
· 全部沒實打（戰鬥交給 Ray）。

# （上一段）截至 `ver 2026.09.22-1778`（-1778：索菈娜 ②⑤ 對調、安雅反擊升階改制、赤爪星、非夢魘粉碎）

**`-1778`**（Ray 四段）
· **索菈娜**：② 海宣星 ⇄ ⑤ 箭頭星（「海宣太便宜，海宣＋箭頭基本就永動」）。⚠ 已點亮的星照**位置**記，舊存檔點過 ② 的現在拿到的是箭頭星。
· **安雅的反擊升階改制**（明晰之夢與夢魘化吃同一套，`partner.counterAtkStep` → `defense.resolveThreat` 的 `fireArgs`）：
  基本＝不論圈色反擊必中、**威力照點到的那一圈**（⚠ 推翻 -974「夢魘化一律紅圈威力」）；
  赤足星 +1 階（黃→橘、橘→紅、紅→全暴擊）；鐵蹄星再 +1（黃→紅、橘→全暴擊、紅→兩倍）。
  `weaponCounter` 多第 5 參數 `opt:{crit|mul}`；`critHit(base, force)`。
  ⚠ 順手修：赤足／鐵蹄的 `lucidSec`（+5／+5 秒）**從 -974 起從沒生效**（fireBuff 只讀卡上的 5 秒）→ `lucidSeconds()`。
· **赤爪星**改成「連續 10 次反擊成功（不限圈色）回填主動技」（`counterReloadActive:10`）：成功＝真的開火；
  沒開火或整發挨打就歸零；跨場累積、換局歸零（`state.counterStreak`，進 sessionCarry）。舊的 `burstAtk:1` 退場。
· **夢境破碎・非夢魘化期間**（`dreamBreakInfo` handler，`context:'board'`，一局一次）：直接進入明晰之夢的增益（反擊必中＋升階），
  **不演 cut-in、不重排敵人計時**（「不重置場上攻擊圈」）。夢魘化期間的上滑照舊是粉碎本體；鍵盤的上滑在夢魘化期間也改成交給粉碎。
· **全部沒實打**（戰鬥交給 Ray 測）：語法與 lint 過。要看的：黃／橘／紅三圈在 0／赤足／鐵蹄下的傷害、赤爪星十連、非夢魘上滑。

# （上一段）截至 `ver 2026.09.22-1777`（-1777：女主收回重新配點）

**`-1777`：收回重新配點**（Ray：「不限地點，怕玩家等級不夠卡在迷宮裡出不來」「重配會把當前該女主的 exp 清零，exp 是零就等於免費」）
· `progress.respecCost／respecGirl`：點亮過的星全熄、紀錄全額退回（① 出場就亮與 ⑨ 條件星不動）；
  代價＝**這一級累積的經驗歸零**（夾回這一級門檻，等級不掉）。
· 整備頁技能表多一顆「重新配點（退回 ◆N）」：按一下亮出代價，3 秒內再按一下才執行；沒有可收的星就不出現。
· ⚠⚠ **順手修的 bug（-1132 起就在）**：`newRun()` 清掉「已點亮的星」那把鑰匙之後，第一次讀它時若女主已有經驗，
  會被當成舊存檔、照等級**免費點亮**前幾顆（實測新局打到 Lv5 ⇒ ②～⑤ 憑空亮、紀錄 0 份）。
  -1776 的補帳讓它更容易踩到。現在 `newRun()` 寫入 `{}`（鐵律 9：主線開始後鑰匙一律插著）。
  ⚠ 已經在這個洞裡的舊局不會自動修（那些星是「已點亮」的正常資料），要乾淨就重開或用重新配點。
· 實測（自己的 port）：Lv5 拿 4 份 → 點 ②④ → 重配退 3 份、本級 700 經驗歸零、Lv5 不變、① 仍亮；門檻上重配免費；之後補帳不重複。
  **按鈕的畫面沒有在瀏覽器看過**（整備頁要在城裡開）。

# （上一段）截至 `ver 2026.09.22-1776`（-1776：女主等級上限 20、①出場就亮）

**`-1776`**（Ray：「照現在的算法，女主的等級上限應該是 20」「讓女主一出場就至少先把第一顆星點掉就好了」）
· `girls.expTo` 改 20 段，**總量不變**（滿級約 5 萬）：第一段 800、每段 ×1.12。紫黑之爪（-1027 校準點）約 **Lv5／手上 4 份**。
· `girls.freeStars:[1]`：① 一律視為已點亮、不花紀錄（`progress.girlStarOn` 唯一那支；`girlBonus` 改問它）。
  `starCost` ＝ `[0,1,2,2,3,3,4,4,0]` ⇒ ②～⑧ 19 份＝升 19 級的 19 份，**滿級剛好全亮**。
· 發紀錄改成「**補到應得的份數**」（應得＝(等級−1)×每級份數，已有＝手上＋已點亮的星花掉的，出場就亮的不算）——
  舊存檔門檻表變了、等級跳上去時，下一次結算自動補齊；以前花 1 份點 ① 的也會退回。
· 畫面上「★幾/幾」改問 `girlStarCount`（9 顆），不是等級上限。
· 實測（自己的 port）：① 亮且 `saintComboMul` 0.5 生效；5000 EXP → Lv5、4 份；Lv20 共 19 份，②～⑧ 全點完剩 0；⑨ 顯示條件未達。
· 試算表同步（方案 A＝定案）。

# （上一段）截至 `ver 2026.09.22-1775`（-1775：Install 回填一律下一場生效）
