...
> · `origin/main` ＝ **`-1771`**（見下一段），工作樹只剩 Ray 自己的 untracked 檔，沒有欠 commit。
...
# HANDOFF — 截至 `ver 2026.09.22-1771`（-1771：約會 +3、小女孩拉高）
...
· **約會場景事件演完 +3**：`OUTING.dateDoneAff:3`，實作在 `modules/town.js` 段落收尾 —— 任何 `withWho` 段落第一次演完，
  ...
  ⚠ 雪都瞭望台安雅 M2 那一拍的 **+5（稿上指定）保留**，所以 M2 那一場是 +5+3＝8 —— 要改跟我說。
