...
export const GAME_CONFIG = {
  ...
  /* ══════════════════════════════════════════════════════════════════════
     ...
     ⚠ 連續戰鬥（城鎮戰五格）是把**原始統計加總後評一次**，不是各場各評
       （ver -601：「戰鬥用時也是要用整場的全部戰鬥總和時間」）。
     ══════════════════════════════════════════════════════════════════════ */
  rating: {
    /* ⚠⚠⚠ **唯一要調的數字**。分數 = 100 −（用時秒 ÷ 敵人總血量）× timeK。
       以 300 血為例（timeK 600）：
         10s → 80 分 S　　17s → 65 分 A　　25s → 50 分 B
         32s → 35 分 C　　42s → 15 分 D
       ⚠ 換算：S 要 `用時 ÷ 血量 ≤ 0.033`（每 100 血 3.3 秒）、A ≤ 0.058、B ≤ 0.083。
