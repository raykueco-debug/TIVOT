...
export const VERSION = 'ver 2026.09.17-1592';
...
export const GAME_CONFIG = {
  ...
  tuning: {
    ...
    /* ⚠ ver -720（Ray：「現在破防攻擊是 80%，改成 100%」）：由 **0.7** 改成 1.0。
       ...
       ⚠⚠ **ver -1329 改成 0.6**（Ray：「單次點擊攻擊力為 0.6」）—— 同一版把操作
         由「點盤面」改成「對著敵人立繪狂點」，而可點次數是**剩餘方塊 ×2**，
         所以整段窗口打得出來的量 ＝ 剩餘方塊 × 2 × 0.6 ＝ **剩餘方塊 ×1.2**
         （舊制是一格一發 ×1.0）。兩個數字是一組的，**只改其中一個會走鐘**。 */
    dmgDualMult:         0.6,   // 雙槍傷害倍率（破防窗口的每一擊）
