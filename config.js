...
export const VERSION = 'ver 2026.09.17-1569';
...
/* ══⚠⚠⚠ 讀取分工（ver -1295，Ray 定案）══════════════════════════════════
   「主頁要讀的只有主頁音樂、logo 跟挑戰要用的資源而已；
     飛行也是進去只讀他需要的，城鎮也是，戰鬥也是。**這是讀取分工問題。**」

   ⚠⚠ 為什麼會變成問題：舊版開機那一批掃 `ASSETS` **全部**的圖，
     其餘的在進主選單那一刻由 `preloadRestImgs()` 用 `new Image()` 全部灌下去。
     `new Image()` 不只是下載，它會**把每一張解碼成點陣圖**，而點陣圖是
     `寬×高×4`，與檔案大小完全脫鉤（WebP 常壓到 1/30~1/50）。實測：

       | 版本 | ASSETS 圖片 | 解碼後 |
       | ver -568（飛行地圖搭起來那時） | 50 張 | **257 MB** |
       | ver -1293 | 132 張 | **738 MB** |

     257 MB 這台塞得下，738 MB 塞不下 —— 實測會**直接把瀏覽器分頁壓死**。
     Ray：「PC 完全跑不動，一開就爆 RAM，之前不會這樣。」他是對的：
     這是退化，不是機器不夠。
   ⚠⚠ **教訓：預載的成本要算「解碼後的像素量」，不是「檔案大小」。**
     ver -344 那段註解寫的「29 張、5.3 MB」量的是**傳輸量** ——
     那 5.3 MB 的 WebP 解開來是 250 MB，當初的取捨是照著錯的尺做的。
   ⚠ 音訊**不在這一刀裡**：整批只有 35 MB，而且 §6.6「音效不載完不放行」
     與 ver -433「story 的 SE 要進開機批」是 Ray 追了三版才定下來的，
     不要順手拆。沒預載到的音效也不會消音 —— `audio.js` 的 `playSrc`
     查不到 buffer 會自己 `load()` 再播，只是可能晚一拍。
   ⚠ 其餘畫面**本來就有自己的門**：飛行頁 `preloadSfx`、劇情頁 `preloadStory`、
     敵人立繪 `loadEnemyPortrait`、城鎮背景 `town.bgFor`、插圖 `resolveCg`、
...
/* ══⚠⚠⚠ **音訊也要分工**（ver -1354，Ray：「我每天都要修一次」「檢查首頁是否只讀
   首頁跟挑戰的資源」）══════════════════════════════════════════════════════
   ver -1295 那一刀**只切了圖**，音訊被明寫「不在這一刀裡」（理由：當時整層只有
   35 MB）。實測 ver -1353 的開機：**SE 74 支 4.91 MB ＋ VO 43 支 0.76 MB**
   ——117 支、5.67 MB，全部在開機揹著，**而且那一段依 -430 不設時限**。
   首頁根本用不到其中絕大多數（受擊音、武器音、四個人的技能語音）。
   ⇒ 那就是「卡在首頁讀取」。

   ⚠⚠⚠ **這不是推翻 -430 的「音效不載完不放行」** —— 那條規矩說的是
     「**這個畫面要的**音效不載完不放行」，不是「全庫音效」。它當初要保的是
     開場的 `se_steps`／`se_Fall`（-433），那兩支在 `story.seSources()` 裡，
     **照舊留在開機等待的那一批**。

   ⚠ 名單用**排除法**（白名單以外一律晚載），與 `LATE_BGM_PATHS` 同一個形狀：
     日後加一支新音效，預設會落在「不擋開機」那一邊 —— 漏寫的下場是「晚一拍」
     而不是「開機多揹一支」（同首頁鈕的白名單，§6.9）。
   ⚠ 晚載不會變成沒聲音：`audio.js` 的 `playSrc` 查不到 buffer 會自己 `load()` 再播
     （§6.6 既有的行為），而且進主選單那一刻就開始背景抓了。 */
export const HOME_SFX = {
  /* 開機**要等**的：讀取頁→主選單這一段真的會響的 UI 音。
     ⚠ 劇情那一批（`story.seSources()`）由 main.js 另外併進來 —— 它不在 ASSETS，
       而且是 -433 明令要等的那一批（開場的跑步與跌倒）。 */
  prefixes: [],
  keys: [
