...
export const HITFX = {
  ...
  /* ⚠⚠⚠ **`cdSec` ＝放光的冷卻，而這裡只是「沒填時的保底」**（ver -1666 立；
     -1667 改由敵人卡決定，Ray：「怎麼沒 CD 啊」「**CD 沒有在 enemie.xls 嗎?**」）——
     它是這一隻的**主動攻擊特效**（`hitFx.assault`），而那隻龍每 3~5 秒攻擊一次
     ⇒ 沒有冷卻的話 2.9 秒的全螢幕白光幾乎連成一片，玩家什麼都看不到。
     ⚠⚠ **逐怪的值在敵人卡上**：`hitFxCd`（秒）＝ `enemies.xlsx` 的「受擊特效／CD秒」
       那一欄（ver -1667 新增），Ray 在 Excel 上直接調。**卡上有值就用卡上的**，
       這一格只在卡上留白時頂著 —— 留著它是因為漏填的下場不可以是「沒有冷卻」
       （鐵律 13：安全的那一側是預設）。
       ⚠ 所以這個 12 與 `bl_dragon_sky` 卡上的 12 是**兩件事**（保底／實際值），
         不是同一個數字寫兩次；改卡上那一格不必回來動這裡。
...
export const VERSION = 'ver 2026.09.22-1776';
...
export const GAME_CONFIG = {
  ...
  /* ══⚠⚠⚠ **飛行畫面的曲子由旗決定**（ver -1433，Ray 兩條）══════════════════
     ...
     ⚠⚠ **為什麼不寫在腳本那一拍上**：飛行頁是**另一個 document**、有自己的一套
       ...
       而 Ray 要的是「進到飛行畫面以後」。
     ⚠⚠⚠ **crisis 那一條 ver -1527 補上 `until`**（Ray：「飛行地圖也是，劇情 bgm
       在劇情結束後就要回到預設 bgm」）══
       · -1433 的理由是「『直到切換到探索地圖』是自然發生的 —— 降落進城時 `town`
         自己的 `bgm`／`bgmWhen` 會接手，不必再記一支旗」。
       · **那句話只算到「降落那一次」為止**：旗是永久的，所以**下一次起飛**
         （以及之後的每一次）`need` 照樣成立 ⇒ 整個遊戲後半段的飛行畫面
         **永遠**是 crisis，而畫面上沒有任何錯誤訊息。
       · ⇒ 這是鐵律 7 但書那條「過期的假設躺在註解裡，沒有人會回頭看它」的又一次：
         **「接手」與「結束」是兩件事** —— 城接手的是**那一格**的曲子，
         它管不到「我下次再飛上天要放什麼」。
       ⚠⚠ 終點取 `bl_night_done`（＝古城入口那一段演完、人被搬回東泊旅店）
         —— 那就是 Ray 原話裡的「切換到探索地圖」那一刻，**用旗把它寫下來**
         而不是靠「自然發生」。
       ⚠ **「沒有終點」的那一條一律是地雷**：這張表由上往下取第一個成立的，
         而旗只會越插越多 —— 加任何一列之前先回答「誰把它收掉？」（鐵律 9）。
     ⚠ crisis 要排在 warhorn **上面**：空中戰打完之後兩條的 `need` 都成立。 */
  flightBgmWhen: [
    { need:'bl_night_sky', until:'bl_night_done', bgm:'crisis' },
  ...
  storyPartnerBy: [
    ...
    /* ══⚠⚠ **會合之後兩位回到池子裡**（ver -1679）══
       上面那兩條用 `not:'tomb_split'` 把她們拿掉 —— 而 `not` 表達不了「窗口」
       （分組之後、會合之前）。⇒ 不去動 `tomb_split`（它同時鎖著 BGM，
       一支旗兩個用途，拔了音樂會跟著跑掉），**改成再加兩條「會合就回來」**：
       `storyPartnerPool` 是由上往下收集＋去重，所以後面這兩條補得回來。
       ⚠ `tomb_rejoin` 由底層梯廳那一段插（蕾娜第一次開口那一拍）。
       ⚠⚠ 順序＝池子的順序，而 `storyPartnerKey` 沒得挑時取第一個 ⇒
         會合之後預設是**安雅** —— 那正好是諾薇兒熔斷之後該遞補的人（見 `partnerBench`）。 */
    { need:'tomb_rejoin', key:'anya' },
    { need:'tomb_rejoin', key:'sorana' },
  ],

  /* ══════════════════════════════════════════════════════════════════════ *
   *  出局中的搭檔（`partnerBench`，ver -1679，Ray：「強制 NI 那一場以後諾薇兒
   *  到走出古墓為止都不能選做夥伴，**選她時確定鈕會顯示熔斷**」）
   * ----------------------------------------------------------------------
   *  ⚠⚠⚠ **「在池子裡但不能配對」與「不在池子裡」是兩件事**：
   *    · `storyPartnerBy` 的 `not` ＝ 她**根本不在**（頁籤上看不到她）
   *    · 這一張表     ＝ 她**看得到、按得下去預覽，但確認鈕變成那三個字**
   *    後者才是 Ray 要的 —— §6.5.5「不要靠藏起鈕擋」：鈕不見了玩家只會以為壞了，
   *    看得到而且被告知原因，他才知道發生了什麼事。
   *  ⚠ 一項：`key`／`need`（哪支旗開始）／`until`（哪支旗解除，可不寫）／
   *    `map`（只在這張地圖上出局，可不寫）／`label`（確認鈕上印什麼）。
   *  ⚠⚠ 「到**走出古墓**為止」寫成 `map:'tomb'` **而不是再發明一支沒人拔的旗**：
   *    離開那張圖這件事本來就有唯一的真相（人現在在哪），
   *    多一支 `tomb_out` 等於多一個「誰來拔」的問題（鐵律 9）。
  ...
  girls: {
    ...
    /* ══⚠⚠⚠ **ver -1776：上限 Lv20**（Ray：「照現在的算法，女主的等級上限應該是 20」）══
       每升一級 1 份《戰鬥紀錄》、①～⑧ 全亮要 20 份 ⇒ 等級要拉長到 20。
       ⚠ **總量不變**（Lv 滿 ≈ 5 萬，同 -1027 的「專養一位約 70 局」），只是切成 19 段：
         第一段 800、之後每段 ×1.12（取整到 50）—— 前期一兩局就升一級（立刻有回饋），後期一段約 6000。
       ⚠ 對照：紫黑之爪（-1027 校準點，約 4300~6200）大約 **Lv5** ⇒ 手上 4 份，
         剛好買得起諾薇兒的 ①先鋒＋②引路＋④堅殼（Ray：「早期就進入穩定的不死狀態」）。
       ⚠ 舊的九段表（-1027）：[0,1500,3800,7000,11500,17500,25500,36000,50000]。 */
    expTo: [0, 800, 1700, 2700, 3800, 5050, 6450, 8050, 9800, 11800,
            14000, 16500, 19300, 22400, 25900, 29800, 34200, 39100, 44600, 50750],

    /* ══⚠⚠⚠ **星要用「戰鬥紀錄」點亮**（ver -1132，Ray 交辦）══════════════════
       > 「女主等級系統修正，**每一次升級會產生一份特殊道具「戰鬥紀錄」**，
       >   女主的星要用戰鬥紀錄點亮，**等級越高的星所需的戰鬥紀錄量越多**，
       >   把 exp 改成戰鬥紀錄」「**戰鬥紀錄要分角色**喔」

       所以現在**一位有三個量**（全部逐角色）：
         ① 累積的戰鬥紀錄（點數）＝原本的 EXP（`expTo` 那張門檻表）→ 決定**等級**
         ② 道具《她的戰鬥紀錄》（`items.defs.rec_<key>`）＝**升一級產出一份**
         ③ 已點亮的星（`progress.girlStars`）＝**花掉②換來的**
       ⚠⚠ 等級到了**不會自動亮星**（這是這一版最大的改變）：等級只是「能不能點」
         的門檻，真的要亮還得花道具 —— `girlBonus` 因此改成只加**已點亮**的星。
       ⚠ 舊存檔沒有③那把鑰匙 ＝ 視為「Lv1~現等級全部已點亮」（不沒收玩家手上
         已經有的能力），見 `progress.girlStarsAll` 的遷移。

       ⚠⚠⚠ **這兩排數字是草案，等 Ray 定**（同護符與 `gunUpgrade.recipes`）：
         · `recordPerLevel` ＝升一級給幾份（1）
         · `starCost[i]`    ＝第 i+1 顆星要幾份（1,1,2,2,3,3,4,4,5，合計 23）
         九級一共只產出 9 份，而九顆全點要 23 份 ⇒ **點不滿，要選**
         （形狀刻意照主武器九星那一套「非線性、玩家自己挑」）。
    ...
    starCost: [0, 1, 2, 2, 3, 3, 4, 4, 0],
    starCond: { 9: 'girlstar9' },
    /* ══ ver -1776：**①一出場就亮**（Ray：「讓女主一出場就至少先把第一顆星點掉就好了」）══
       Lv20 升 19 級＝19 份，②～⑧ 剛好 19 份 ⇒ 滿級正好全亮（原本 ① 要 1 份，會差一份）。
       亮不亮只有 `progress.girlStarOn` 一支在答（鐵律 7），這裡列的星一律當作已點亮、不花紀錄。 */
    freeStars: [1],
  ...
  items: {
    ...
    defs: {
      ...
      /* ══⚠⚠⚠ **《戰鬥紀錄》—— 逐角色**（ver -1132，Ray：「每一次升級會產生一份
         特殊道具『戰鬥紀錄』」「戰鬥紀錄要分角色喔」）══════════════════════════
         她升一級就入袋一份（`progress.addGirlExp` 那個唯一的發放點），
         拿去整備頁的技能表點亮她的星（成本見 `girls.starCost`）。
         ⚠⚠ **名字不寫死**：只標 `girl:'<key>'`，顯示名由 `inventory.nameOf` 用
  ...
  tutorial: {
     ...
     *  dualGo     ＝雙槍破防 cut-in 結束後（ver -1329 起：接著玩家**對著敵人立繪狂點**）
  ...
  battles: {
    ...
    /* ⚠⚠⚠ **ver -1130 收斂成一條全域規則**（Ray：「stg1 以後打靶之外每場必出評價，
       就這麼簡單」）：這一場**不再自己寫** `noEvalBeforeStage` ——
       「第 0 章不評」現在是 `script/evaluation.js` 的 `FROM_STAGE`（全域一處），
       打靶則是卡上的 `noEval`。兩層加起來就是 Ray 那一句話。
  ...
  tuning: {
    ...
    /* ══⚠⚠⚠ **只准調暗，不准調亮**（ver -1565，Ray：「我有角色立繪隨場景光調整
       明度的規矩 但**只可調暗，不要調亮**」）══ 兩個上限都夾在 **1.00**。
       ⚠ 為什麼：立繪本身就是畫好的成品，**它的亮度是美術定的** —— 往上推等於
         替美術重新上色，白衣服會過曝、線稿會被洗掉。往下壓只是「她站在暗處」，
         那是物理上讀得通的事。
       ⚠ 所以很亮的場景現在是「立繪不動」而不是「立繪跟著亮」。要讓她在雪地裡
         不顯得暗，是**調 `min`** 的事（下限放寬），不是把上限拉高。
       ⚠ 舊值 `max:1.08`／`satMax:1.05` 留在這行註解裡當紀錄。 */
    portraitTone: { ref:0.45, gain:0.90, min:0.85, max:1.00,
                    satGain:0.35, satMin:0.90, satMax:1.00 },
    ...
    /* ══⚠⚠ **BR（雙槍破防）的份量：次數＝殘格、每一擊＝一發普攻**
       （ver -1595，Ray：「把 BR 的次數改成跟殘格一致，攻擊力預設同普攻，
         留升級空間」）══════════════════════════════════════════════════════
       · `dualTapsPerCell:1` ⇒ **可點次數 ＝ 發動當下還沒點掉的格數**（一格一發）。
       · `dmgDualMult:1.0`   ⇒ **每一擊 ＝ 一發普攻**。
         ⚠ BR **不吃暴擊也不吃 `atkBuff`**（見 `combat` 那一行，刻意的安全牌）——
           所以 1.0 的意思是「與一發**沒有暴擊**的普攻等值」，不是兩倍普攻。
       ⚠⚠ **這兩個數字是一組的，只改其中一個會走鐘**：整段窗口打得出來的量
         ＝ 殘格 × `dualTapsPerCell` × `dmgDualMult`。
         歷代：-720 一格一發 ×1.0 ＝**殘格 ×1.0**／-1329 改成對著立繪狂點，
         殘格 ×2 × 0.6 ＝**殘格 ×1.2**／**-1595 回到殘格 ×1.0**（＝「同普攻」）。
       ⚠ **升級空間就是 `dmgDualMult` 這顆旋鈕本身**，而且已經有一條乘法掛鉤：
         `prog.girlBonus(搭檔,'brDmgMul')`（女主的九級／星）——
         日後要給雙槍九星或護符加 BR 傷，接在那一條上，**不要再開第二個倍率**（鐵律 7）。
       ⚠ 畫面上同時最多幾個瞄準點是另一件事（`brAimMax`），與份量無關。 */
    dmgDualMult:         1.0,   // 每一擊＝一發普攻（不吃暴擊／atkBuff）
    dualTapsPerCell:     1,     // 可點次數＝發動當下的殘格數
    ...
    fileGain: {
      ...
      se_bulletsfly1:1.259, se_bulletsfly2:1.304,   // CAP
      se_bulletsfly3:1.259, se_bulletsfly4:1.282,   // CAP
    ...
    nightmare: {
      ...
      /* ══⚠⚠⚠ **ver -1696（Ray 定案）：夢境粉碎改成「強制消去敵最大 HP 的 20%」** ══
         > 「把 dreambreaker 改成**強制**消去敵最大 hp 的 20%（不是現有 hp，是 hp 上限的
         >   20%），敵若遭此技 hp 歸零**等同 execute**」

         兩件事一起改，**上面那一整段對這一招的說明已作廢**（留著當紀錄）：

         1. **不再乘「清掉的格數 ÷ 16」** —— 一律 20%。`burstPct` 由 0.25 降到 **0.20**。
            ⚠ 推翻 ver -897 的比例算法（「點掉的格子越多炸的傷害越高」）。
              換來的是**一個固定得可以規劃的份量**：玩家知道這一招就是兩成，
              不必先算自己清了幾格。⚠ 回血那一條**照舊吃比例**（它本來就是
              「視你在 NI 打掉的格數而定」，而且現在是 0）。
         2. **`burstFloor` 歸 0 ＝ 它現在打得死** —— 推翻 ver -673／-689 的
            「炸不死也沒關係，最後留個 10%／5%」。歸零那一下**等同處決**
            （`markExecution` ＋ 賺回一次 Install ＋ 回滿血，見 `saint.niBurstResolve`）。
            ⚠ 欄位留著寫 0 不刪（`saint.js` 是 `NI_BURST_FLOOR>0` 才夾）——
              要把「打不死」加回來只要填一個數字（鐵律 1）。
         ⚠ 「界心星」（安雅 Lv8 的 `burstLastCell`，剩最後一格 30%）**不受影響**：
           那是一顆星的覆寫，30% 本來就大於 20%。 */
      burstFloor: 0,
      burstPct: 0.20,
    ...
    /* ══⚠⚠⚠ **怪的等級 × 類型 × stage 加成**（ver -1582，Ray 定案）══════════════
       > 等級分 SABCDE：S 強力boss／A Boss／B 中boss／C 略強小怪／D 弱小怪／
       >   E 劇情敵（一概先套帝都賞金獵人數值）
       > 類型：速度型 攻擊頻率 3（hp 較低）／力量型 4（hp 中等）／
       >   防禦型 5（hp 高，**BR 統一增傷 50%**）／**無分類的由我手動設製**
       > 「怪的強度係數有**天生的等級加成**，還有**後天的 stage 加成**」

       ⚠⚠⚠ **卡上存的是絕對值**（§6.5.2 的鐵條：卡上寫絕對值就存絕對值）——
         這張表是**算基準的那一把尺**，不是執行期再乘一次。`tools/enemies_baseline.py`
         用它把 hp／attack／assaultEvery／boardGrids 算出來寫進卡；stage 加成才是
         執行期的（見下面 `stageCurve`）。
       ⚠ 沒有 `tier`／`atype` 的卡（`atype:''`）＝**Ray 手動設製**，基準工具一律跳過。

       **基準是量出來的，不是拍的**：素身（不算女主升級／雙槍九星／副武器改裝／護符）
       全對不失誤時，`dmgBase 3 ＋ 每連擊 0.2`（上限 20）＋ 暴擊（率與加傷都
       隨 `critCombo` 走，而 `critCombo` **每盤歸零**）⇒ 一輪盤面的累計期望傷害：

           9 格盤 ×5     ：35 ／ 86 ／150 ／214 ／279
           9,9,9,9,16   ：35 ／ 86 ／150 ／214 ／330   ← C 級
           9,9,9,16,16  ：35 ／ 86 ／150 ／266 ／381   ← 中 boss 以上（一輪 59 下）

       所以 `hp` 這一欄就是「**打幾盤會死**」：
         D 2 盤／C **4~5 盤**／E 賞金獵人原值／B **整整一輪（5 盤）**／A 8 盤／S 12 盤。
       ⚠ `atk` 這一欄是「**全程不擋的話幾下死**」（玩家 100 血，持久 HP）：
         D 12 下／C 8 下／B 6 下／A 5 下／S 4 下。 */
    enemyTier: {
      /* hp＝基準血量（力量型＝×1）；atk＝攻擊力；grids＝盤面序列；
         stack＝BR 圈可不可以疊（1 開 0 關，**C 以下預設不疊**，Ray 指定）。
         ⚠ `D 以下不出 16 格`（Ray 指定）—— D／E 一律 9 格盤。 */
      /* ⚠⚠ **16 格不要太多**（ver -1582b，Ray：「不要太多 16 格，會降低速度感與
         爽快感」）：中 boss 以上 `9,9,9,16,16`、C 只有**最後一盤**出 16、
         D 以下完全不出（Ray 原話「999 16 16」「只有最後一盤出 16」）。 */
      S: { hp: 900, atk:25, grids:[9,9,9,16,16], stack:1, desc:'強力boss' },
      A: { hp: 560, atk:20, grids:[9,9,9,16,16], stack:1, desc:'Boss' },
      B: { hp: 380, atk:16, grids:[9,9,9,16,16], stack:1, desc:'中boss' },
      /* C＝**約 4~5 盤死**（Ray 指定）：`9,9,9,9,16` 的第 4 盤末是 214、第 5 盤末 330
         ⇒ 270 落在第 5 盤中間。⚠ 16 格拿掉之後這條曲線整個變了，**不能沿用舊的 200**
         （那在新序列裡是第 4 盤初就死，只有 3 盤多）。 */
      C: { hp: 270, atk:12, grids:[9,9,9,9,16],  stack:0, desc:'略強小怪' },
      D: { hp:  90, atk: 8, grids:[9,9,9,9,9],    stack:0, desc:'弱小怪' },
      /* E＝劇情敵：**一概先套帝都賞金獵人**（`guild_hunter`）的數值（Ray 指定）。 */
      E: { hp: 200, atk:10, grids:[9,9,9,9,9],    stack:0, desc:'劇情敵' },
    },
    /* `every` ＝攻擊頻率（秒）。卡上的 `assaultEvery` 是 `[下限,上限]`，
...
/* ⚠⚠⚠ **預載頁的時機與那一批載什麼，唯一準則在 CLAUDE.md 鐵律 13**（ver -1579）——
   那五條是 Ray 的原話。**這裡不重寫理由**：各支檔案各寫一份必然過期，
   而過期的理由沒有人會回頭看（-1578 就照著一段 -934 的舊註解做錯了一次）。
   下面只寫「**這張表是什麼**」。 */
...
export const HOME_SFX = {
  /* 首頁那一批的音效白名單（鐵律 13 的第 1 條）。
     ⚠ 劇情那一批（`story.seSources()`）不在 `ASSETS`，由 main.js 另外併進來。 */
  prefixes: [],
  keys: [
    'se_general_click',   // 所有鈕
    'se_pageflip',        // 翻頁
    'sfx_startbt',        // 出陣鈕
    'sfx_start',          // 起始
  ],
...
export const ASSETS = {
  ...
  */
  ...
  /* ══⚠⚠ **子彈火線的飛行聲**（ver -1622，Ray：「音效 Se_Bulletsfly，做成略有變化的
     ...
     （速率 1.000／0.975／1.022／1.040 ＋ 微幅增益 —— 音高與長度一起動，
     讀起來像不同支槍管／不同距離），原檔已回收。
     ⚠⚠⚠ **尾端的「擊中血肉」音已淡出並截掉**（ver -1626，Ray：「把血肉音淡出」）：
       頻譜上 0.64 秒之後質心掉到 70~1000 Hz、低頻佔比 75~99%（前面那一段是
       3000~6000 Hz 的飛行咻聲）—— 餘弦淡出 0.62→0.85 秒，0.88 秒截斷，
       全長由 1.019 秒變 0.880 秒。
     ⚠⚠ **變化要「略有」＝半音差控制在 ±0.7 以內**（ver -1622b）：
...
/* 咬痕特效圖（ver -761，Ray 交件）：CSS 背景用同一路徑，這裡登記的是**路徑**
     （`asset()` 查得到）。⚠ 預熱是戰鬥那一邊的事（`combat.warmBattleFx`）——
     登記本身不載任何位元組（鐵律 13）。 */
