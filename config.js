...
export const VERSION = 'ver 2026.09.09-993';
...
export const GAME_CONFIG = {
  ...
  partners: {
    ...
    anya: {
      install:{ name:'夢魘化', en:'NIGHTMARE INSTALL',
        ...
        desc:'敵人框右滑發動（每場一次）：體力由當下的量持續流失（滿體力約 13 秒，'
            +'體力越少越短），抽乾即熔斷（HP 剩 1）；期間所有反擊命中 100%。'
            +'流失殆盡前清空 16 格＝體力回到發動時的量，並以期間總傷的兩成追加一擊。'
    ...
    /* ══⚠⚠ **升級陡度**（Ray：「你先拿捏」）══ 累計 EXP 門檻，索引＝等級−1。
       ...
       **怎麼拿捏出來的**（數字都可以直接調，形狀不必動）：
       · 一局拿多少 ＝ `rating.exp` 那個式子：`137 + 分數×8.7 + 微擾 + overkill×3`
         ...
         **平均一局 600 上下**（連戰是整局併帳算一次，不是逐隻）。
       · 所以 **Lv9 ＝ 20000 ≈ 專養一位 33 局**；S 級打法（1000/局）只要 20 局。
       · 三個人輪流養 ⇒ 全部滿級需要 **約 100 局**（S 級打法約 60 局）。
      ...
      anya: [
        ...
        { star:'κ Geminorum', skill:'install',     name:'孿生星',
          ...
          /* 實作＝把那一段的總長延長 0.5 秒（抽血是「從起點線性到 1」，
             ...
             而且不必另外做一套暫停／續跑的狀態）。
             ⚠ 「一次反擊」＝`weaponCounter` 真的開火那一次（三帶都算，同鐵律
  ...
  rating: {
    ...
    /* ⚠ `coop` ＝**獵手的共鬥發動一次加 10 秒**（ver -958 立 5、**-959 由 Ray 改 10**）——
    ...
    penalty: { wrong: 2, assault: 3, block: 1, delay: 1, counter: -0.5,
               overkill: 0, perfectBoard: -1, maxBurst: -10, execution: -15, coop: 10 },
