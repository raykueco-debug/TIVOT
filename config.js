...
export const VERSION = 'ver 2026.08.29-568';
...
export const GAME_CONFIG = {
  ...
  /* ══════════════════════════════════════════════════════════════════════
     ...
     ⚠⚠ **這一段就是給 Ray 調的**：改係數不必動程式。三組旋鈕：
       · `secPerHp`  **唯一的時間係數** —— 每 1 點敵人血量給幾秒
       · `penalty.*` 各種失誤／表現值多少秒
       · `tiers[].max` 每一級的門檻（ratio）
     ⚠⚠ **Boss 沒有額外加秒**（ver -602，Ray：「boss 不用額外加秒，現在都用 hp
       來控」）：難度由**敵人卡的 HP** 表達，時間基準就跟著血量走 ——
       再給 Boss 一個獨立的加成等於同一件事調兩個地方（鐵律 7）。
       所以 `bossBonus` 整個拿掉了，`isBoss` 也不再參與評價。
