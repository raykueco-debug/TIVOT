...
export const VERSION = 'ver 2026.09.01-723';
  ...
  rating: {
    ...
    /* ══⚠⚠ 等第 → 好感（ver -723，Ray：「評價A好感度也給一半。跟別人相反的
       ...
       也算進去，一律**給一半**：
         · 搭檔（諾薇兒／安雅）  S +1、**A +0.5**
         · 索拉娜（方向相反）    D +1、**C +0.5**
         · 蕾娜（不是搭檔，最難） S +0.25、**A +0.125**
       ⚠⚠ 蕾娜走**點數**不走小數：`addAffection` 對齊到 1/4（`Math.round(v*4)/4`），
         0.125 會被四捨五入掉。所以她記的是 `renna.pts` 的點，滿 `renna.per` 點才 +1
         —— S 記 2 點、A 記 1 點、每 8 點 +1，換算回來正好是 +0.25／+0.125，
         而且與 -557 的「四次 S +1」完全等值（4×2＝8）。
       ⚠ 沒有 E 那一格（Ray 指定沒有 E，`tiers` 最後一級是兜底）。 */
    affection: {
      partner: { S:1, A:0.5 },
      sorana:  { D:1, C:0.5 },
      renna:   { pts:{ S:2, A:1 }, per:8 },
    },
