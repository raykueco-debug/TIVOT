...
# 十一、⭐⭐ 索拉娜的正規產線定案：**Ray 出 SD 臉 → 本機 ToonOut**（ver -1670 後）

> Ray：「看來索可以用 **SD 跑再 toon 掉**，挺穩，我再選一些 **gpt 臉**的丟 alpha 資料夾」

⚠⚠⚠ **這一條把「內容判定擋死怎麼辦」整個解掉了** —— 索拉娜那套深膚舞者裝
幾乎張張踩線（`laugh` 連擋 12 次），而這條路**根本不經過任何審查**：
臉由 Ray 在本機出，去背也在本機。**日後被擋的張數一律走這裡，不要再盧提示詞。**

## `resources/si/alpha/` ＝ 常駐投放夾

Ray 把白底稿丟進去，美術收件。
⚠⚠⚠ **收完只清內容、資料夾留著**（Ray 明講）——
ver -1670 後我把整個資料夾回收掉了，隔一句話就被要求還原。
現在有 `README.md` 入版控（讓資料夾活過 clone）＋ `.gitignore` 擋內容。
原 PNG 移進 `resources/_originals/si/<鍵>_src.png`。

## ⚠⚠⚠ 收件的第一件事：**量「他改了哪裡」，不是去背**

| 他重畫的 | 判準 | 作法 | 實例 |
|---|---|---|---|
| **臉／內部** | 改動像素落在 `α<200` 的**有 0 個** | **alpha 一個位元都不動**，只換 RGB | battlecry ×6 |
| **頭髮／輪廓** | 改動像素**碰得到**半透明區 | 跑 ToonOut，只取那一塊 | `laugh` |

**判錯的代價是不對稱的**：對沒動過的輪廓重解 alpha ＝ 純粹的風險
（實測髮緣接縫帶差到平均 6.7／最大 228），而漏跑 matting 會當場看到白邊。
⇒ **量完再決定，不要照抄上一次。**
