...
## 八、2026-09-24 晚（Windows 那台）：米夏 `draw` 護手 —— ✔ 交件（第三版）

- 退兩次才過：v1 只黏弓的末端；v2 的 S 形鍔尖仍在拳頭上方（Ray：「手不可能握在護弓之上」）。
  v3 把「柄頭→柄（拳頭）→整片鍔含鍔尖→刀身」的**畫面方位**寫死、並獨立點名鍔尖的位置才過。
  提示詞寫法、數字、驗收教訓在 `si/_misha_program_worklist.md` **附三**。
...
- 米夏待修剩下：`close` 自然站姿 → `closeopen` 重做；刀長不合格 7 張等 Ray。
  ⚠ 那 7 張若重畫，**鍔與鍔尖的位置照附三那段寫**，不然會再犯同一個錯。
