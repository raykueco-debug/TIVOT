...
- ⚠⚠ 上面那組座標是 **`tools/map_check2.py` 量出來的**（`resources/map/_spots_santasofia.json`
  是同一份），**不要用眼睛在圖上估**、也不要沿用別張圖的。
- ⚠ **第一次上線不必掛 `?v=`**（新增不是同名覆蓋）。日後同名覆蓋這張圖就一定要加／改
  `?v=N`，而且**整組一起帶**（§5）。
- 驗收：12 顆墨點全找到、11 條邊有 10 條在同一塊墨裡

- ⚠ `midtown—square` 在像素上**差一線沒接起來**：那條直線往下走到**噴泉**就停了，
  噴泉再連到 Square 的墨點 —— 中間有一道十幾 px 的白。畫面上讀起來是「路走到廣場的
  噴泉」，玩家看得出是通的，所以我收下了；要完全乾淨就得再擲一次整張圖
  （而那會讓十二顆墨點全部位移、座標要重量）。**這是已知的、刻意留下的**，
  不是漏驗 —— 日後誰再跑一次檢查器看到它，不用當成新 bug。
