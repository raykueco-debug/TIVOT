...
- [x] **`map_fallen.webp`（石製遺跡，5 格）＋ `map_santasofia.webp`（聖索菲亞城，12 格）**
      ...
      · **佈局照 `tools/map_layout.py` 產的權威圖**（`_layout_fallen.png` / `_layout_santasofia.png`），
        `POS` 這一輪補了兩組；聖索菲亞**直接沿用 `capital` 那一組**（節點 id 一樣，方向驗證自然過）。

      ### ⚠⚠ 這一輪學到的四件事（下一張照做）

      1. **座標要逐格寫進提示詞**（「Altar (0.46, 0.13)」那種），模型會照著放，
         誤差 10~60px。⚠ 然後**把那組座標存成 `_expect_<id>.json`** ——
         檢查器拿它當「該在哪」的基準，比從 `POS` 的格點推準得多
         （推的那一版誤差到 670px，貪心配對當場把 midtown 配到左上角的一塊撕邊，
         **十一條邊報了八條斷線，而畫面上那八條線好端端地在**）。
      2. **⛔ 不要寫「串珠感」**。寫了之後直向的連線被畫成很淡的點狀，縮小以後
         讀起來像「沒有路」。正確的寫法是：**橫的直的同一個粗細、同一個墨色，
         不可以斷開；唯一的虛線是通往錨的那一小段**。
      3. **⛔ 速寫記號不可以騎在連線上**。第一版聖索菲亞的拱門跨在十字路口正中央，
         把上下兩條路從中間切斷。寫死：**記號一律畫在自己那顆墨點的斜上方，
         整個記號不碰到任何一條線**。
      4. **「alpha 去背」它可能畫成棋盤格**（第二版就是：RGB 圖上畫了灰白格子來
         *表示*透明）。所以提示詞要多一句「⛔ 不要畫灰白相間的棋盤格來表示透明」，
         而且**收圖後一定要量 alpha**，不要假設（同 -909 那條「每次都要重新確認底色」）。

      ### 去背的三種底，各有各的鍵法
      | 這一次回的底 | 怎麼鍵 |
      |---|---|
      | 深褐暈影（不透明） | 亮度門檻 150 ＋ 四邊 flood fill |
      | 畫出來的棋盤格（不透明） | **彩度**門檻（`R−B < 22`＝灰）＋ 四邊 flood fill |
      | 真 alpha | 什麼都不要做 —— ⚠ 先 `convert('RGB')` 會把它毀掉 |

      ### 已知、刻意留下的一處
      · 聖索菲亞的 `midtown—square` 在像素上差一線沒接起來（直線走到噴泉就停，
        噴泉再連到墨點，中間十幾 px 的白）。畫面上讀得出是通的，重擲整張的代價是
        十二顆墨點全部位移、座標要重量 —— 所以收下。**不是漏驗。**
