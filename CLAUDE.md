...
- ⚠ **縮放與站位是兩個獨立的旋鈕,各鎖各的**:
  - **縮放鎖「單眼寬」**。眼睛是全體共通的解剖尺度,只有讓螢幕上的眼寬一致,大家才像同一台相機拍的。`eye` 是**這張插畫裡單眼的來源像素寬**(原圖 1024×1536 上量的):安雅 34、索拉娜 27、諾薇兒 36。
  - **站位鎖「身高」**。頭頂的螢幕 y = `頂線 + (最高身高 − 自己身高) × 每公分像素`;每公分像素由「真人單眼寬 ≈ 3cm」換算,不必另挑係數。
- ⚠ **兩者不可共用一個旋鈕**(舊版的 `h = cm × CAST_H_PER_CM` 就是)。畫風會把身高差吃掉:安雅的眼睛畫得比索拉娜大 26%,共用旋鈕時反而變成安雅比較高。
