...
- ⚠ **`h` 是從身高算出來的,不要手填**:`h = 身高cm × CAST_H_PER_CM`。手填遲早會改了一個忘了另一個,畫面上就變成身高差時有時無。插畫本身給不出身高(都是 head-to-toe 全身框,實測三張人物像素高差不到 1%),所以身高只能由設定決定。
- ⚠ 基準錨在**最高的角色**(索拉娜 176cm = 0.95),不是錨在最矮的 —— 錨最高的才保證誰都不會頂到畫面上緣被切頭。
