...
- ⚠ **縮放與站位是兩個獨立的旋鈕,各鎖各的**:
  - **縮放鎖「單眼寬」**。眼睛是全體共通的解剖尺度,只有讓螢幕上的眼寬一致,大家才像同一台相機拍的。`eye` 是**這張插畫裡單眼的來源像素寬**(原圖 1024×1536 上量的):安娜 34、索拉娜 27、諾薇兒 36。
  - **站位鎖「身高」**。頭頂的螢幕 y = `頂線 + (最高身高 − 自己身高) × 每公分像素`;每公分像素由「真人單眼寬 ≈ 3cm」換算,不必另挑係數。
- ⚠ **兩者不可共用一個旋鈕**(舊版的 `h = cm × CAST_H_PER_CM` 就是)。畫風會把身高差吃掉:安娜的眼睛畫得比索拉娜大 26%,共用旋鈕時反而變成安娜比較高。
- ⚠ 側面圖**不能拿自己的眼寬當尺**:四分之三側面的眼睛被壓扁(索拉娜 side 只有 26px),照量會把她放大成巨人。改量同一角色的 front(兩張插畫的身體佔幅一致,尺可以互通)。
...
⚠ **蕾娜(Regine)≠ 蕾妮(Renee)**。蕾妮是現行搭檔(即死防禦/生命歸還);蕾娜是設計
  文件裡的監察官線女主,程式裡還不存在。現行唯一監察官是芙蕾雅(Freya)。
  「蕾娜是否等於芙蕾雅換名」**未定案** —— 動監察官結算前先問 Ray,不要自己選一邊。
