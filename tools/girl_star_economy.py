...
rows = [
    ...
    ('索菈娜', '②海宣 ＋ ⑤箭頭 ＋ ⑦聚落 ＋ ①地弓', '1+3+4+1＝9',
     '⑦聚落「主動技回填共鬥」是**唯一還能在同一場再開一次**的變身：戰吼→主動技→共鬥，Boss 戰可以開好幾次無敵共鬥。',
     '高：現在三人裡唯一的例外，Boss 戰差距最大；又有評價寬限＋EXP 反向（升得最快）。',
     '把 ⑦聚落也改成「下一場才生效」（與 SI／NI 同一條規則，走同一對 defer／apply）；②海宣（回填主動技）可以留著當她的同場循環。'),
]
...
wa.cell(7, 1, '前提（ver -1775）：聖徒化／夢魘化一場限一次，回填一律下一場才生效；星不用等級鎖，價格 1/1/2/2/3/3/4/4＋⑨條件。'
              ...
              '現在唯一的例外是索菈娜 ⑦聚落（同一場再開共鬥），建議比照同一條規則。諾薇兒的免死組在方案 D 約 Lv5 可成形，但 Boss 戰只有一次免死。').alignment = WRAP
