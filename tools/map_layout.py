...
POS = {
  ...
  'tomb': {
    # ══ ver -1640：**照 -1634 那一版加房間**（Ray：「用上一版加圖」）＋
    #   往三層梯廳的那一段改成**一路往下**（Ray：「人直覺不會往回走」）。
    #   ⚠ 全部正交、不交錯、不斜線（Ray 指定）：一層佔欄 2~12、往下那一串佔欄 14~16。
    # ── 三層（往下）────────────────────────────────────────────────────
    'landing3':(16,12),
  ...
  'canyon': {
                    'altar':(0,0),
                    'bones':(0,1),  'bridge':(1,1),
                    'corridor':(0,2),
                    'entry':(0,3),
  },
  # ── 夏爾森林（ver -1671）：10 格・一條蛇行主線 ＋ 三條一步死路，環 0 ────
  #   入口→林間空地→淺灘→舊獵道→高地→斷崖邊→遺蹟入口（跨圖）。
  #   死路：獸巢（空地右）／河谷（淺灘左）／岩窟（舊獵道左）。
  'shinier_forest': {
                                    'ruins':(2,0),
                    'high':(1,1),   'cliff':(2,1),
    'cave':(0,2),   'trail':(1,2),
    'valley':(0,3), 'shoal':(1,3),  'glade':(2,3),  'nest':(3,3),
                                    'entry':(2,4),
  },
}
# 跨圖出口畫成一個虛線框（`@<圖>:<格>` → 標題）
