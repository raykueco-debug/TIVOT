...
POS = {
  ...
  # ── 平原古道（ver -1460 重排）：11 格・11 邊・環 1 ────────────────────
  #   環的兩半**不等長**：草海 →碎石坡→風蝕岩（2 步，短）
  #                     草海 →石塚群→枯木林→風蝕岩（3 步，長，但路上有古井驛）
  #   ⚠ 環數上限就是 1 —— 這 11 張圖的出口總數是 24（扣兩個跨圖＝11 邊），
  #     `環 = 邊 − 節點 + 1 = 1`。要更多環只能再要「側面有開口」的圖（憲法 §6.5.4.3）。
  'plainsroad': {
    'ravine':(2,0),          # 城外（最後一格・安全區・結算點）
    'gorge':(2,1),           # 狹窄溪谷（倒數第二）
    'windrock':(2,2),        # 兩條路在此匯合
    'scree':(1,3),  'deadwood':(3,3),
    'sea':(1,4),    'cairn':(3,4),  'well':(4,4),
    'beacon':(0,5), 'stone':(1,5),
    'entry':(1,6),
  },
