...
POS = {
  ...
  # ── 平原古道（ver -1461，Ray：「不用環　做岔路跟死路」）：11 格・10 邊・**環 0** ──
  #   整張是一棵樹 ⇒ 走錯就是真的要原路退回來（有環的話會自己接回去，罰不到）。
  #   對的路：道口→里程碑→草海→碎石坡→風蝕岩→狹窄溪谷→城外（6 步）
  #   死路三條：烽燧臺（1 步）／石塚群底下的古井驛與枯木林（各 2 步，整個是死區）
  'plainsroad': {
    'ravine':(2,0),          # 城外（最後一格・安全區・結算點）
    'gorge':(2,1),           # 狹窄溪谷（倒數第二）
    'windrock':(2,2),
    'scree':(2,3),
    'deadwood':(4,3),        # ✗死路
    'sea':(2,4),    'cairn':(4,4),  'well':(5,4),   # ✗死路
    'beacon':(0,5), 'stone':(2,5),
    'entry':(2,6),
  },
  ...
  'tomb': {
    # ══ ver -1638：二層主幹改成**一路往下**（Ray 指定），梯廳→第二道階梯 10 步。
    #   ⚠ 一層照舊往上（列號變小＝前進），二層／三層往下 —— 兩段的方向刻意相反。
    #   ⚠ 一層佔 欄 4~12、二三層佔 欄 13~15，欄不重疊所以不會撞。
    # ── 一層（21 格，前進＝up）──────────────────────────────────────────
    'scriptorium':(4,12), 'chantry':(6,12), 'crossing':(8,12), 'chapel':(10,12), 'confessio':(12,12),
    'tombniche':(4,13), 'aisleW':(6,13), 'nave':(8,13), 'cryptA':(12,13),
    'charnel':(4,14), 'vestibule':(8,14), 'lapidarium':(10,14), 'ossuaryA':(12,14),
    'gate':(8,15),
    'sacristy':(10,10), 'wellyard':(6,10), 'reliquary':(8,10),
    'stair1':(4,11), 'cloister':(6,11), 'apse':(8,11), 'ambulatory':(10,11),
    # ── 二層（21 格，前進＝down）────────────────────────────────────────
    'landing2':(14,12),
    'cistern':(13,13),   'hall2':(14,13),
    'waterstair':(13,14),'colonnade':(14,14), 'mason':(15,14),
    'sump':(13,15),      'corr2':(14,15),
                         'rotunda':(14,16),   'censerroom':(15,16),
    'kiln':(13,17),      'nichehall':(14,17),
                         'sarcE':(14,18),     'shaft':(15,18),
    'urnvault':(13,19),  'ossuary2':(14,19),
    'ashpit':(13,20),    'ossuary':(14,20),   'graveldrop':(15,20),
                         'cryptwalk':(14,21),
                         'stair2':(14,22),
    # ── 三層（6 格，前進＝down）─────────────────────────────────────────
    'landing3':(14,23),
    'gallery3':(14,24), 'vaultW':(15,24),
    'crypt':(14,25),    'bonepit':(15,25),
    'lowaltar':(15,26),
  },
