...
function renderLine(){
  ...
  /* ══ 條件拍（`onlyIf` / `skipIf`，ver -656）══════════════════════════════
     ...
     ⚠ 條件是 progress 的旗標（唯一的真相），不接受任意運算式：
       腳本是資料，資料裡不放程式。 */
  /* ⚠⚠ **寫成陣列＝「任一支成立」**（ver -1484）：`onlyIf:['a','b']` ＝ a 或 b 立著
     才演、`skipIf:['a','b']` ＝ 其中一支立著就跳過。
     ⚠ 單值的行為一個字都沒變（向後相容）。
...
/* 這一段（整條 scene 鏈）要用到的圖／音效／音樂。
   ...
   ⚠ 「什麼時候跳讀取頁」不在這裡判斷，看 CLAUDE.md 鐵律 13 那五條。 */
...
function preloadStory(startId, onProgress){
  ...
  /* ⚠⚠ **音效先載，圖片後載**（ver -346，Ray：「腳步聲跟跌倒音沒載到」）。
   ...
   ⚠ 順序＝**音效 → 圖 → BGM** 三段串接，總上限**只罩後兩段**（§6.6）。 */
...
/* ══ 換場**唯一**的那道門 —— 先放掉上一個畫面的，再只載這一段要的 ══
   ...
   ⚠⚠ 順序照 §6.6：**音效 → 圖 → 音樂**，音效那一段**沒有時限**；
     總上限只罩後兩段 —— 罩到音效就是那條規矩的破口。
