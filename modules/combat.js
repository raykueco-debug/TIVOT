...
export function setup(){
  ...
  enemy.init({ startIntruderFight, updateBars, screenShake, roarBlast,
    ...
    onEnemySet: onEnemySet });
}
/* ══ 換了一隻怪（＝一「場」，§0.5 用詞）的匯流點 ══════════════════════════
   ⚠⚠ **這裡是那件事的唯一時刻**：開場、連戰換敵、Boss 亂入全部經過 `enemy.setEnemy`。
     凡是「每換一隻怪就要做一次」的事都掛這裡，不要各自去找 setEnemy（鐵律 8）。 */
function onEnemySet(){
  partner.onEnemySet();          // 即死防禦 reload（ver -888）＋明晰之夢重新上膛（ver -693）
  /* ══⚠⚠⚠ **副武器的輪轉每一「場」歸位一順位**（ver -1292，Ray：「武器切換模式的
       輪換每一場都會回歸玩家設定的順序的第一把槍」）══
     ver -983 只做到**每一局**（`resetWeaponSwitch` 掛在 `startGame`）—— 連戰打完
     第一隻換第二隻時，手上是上一場最後切到的那一把，於是「這一場帶哪把槍」
     由上一場的最後一次切換決定，玩家在整備頁排的順序做不了主（那正是 -983 要修的病，
     只是當時漏了連戰這一半）。
     ⚠ 照 §0.5 的問法：它在**換一隻怪**時歸零 ⇒ 這是「場」的資源，掛在這裡才對。
     ⚠ 只有本篇（`storyMode()`）與 `fixed` 模式 —— 與 -983 同一條守門，理由見那裡
       （試玩版的出擊整備是玩家在開打前那一頁直接挑的，蓋掉等於丟了那一頁的選擇）。
     ⚠ 走 `weapon.resetToFirst()` 而不是整支 `resetWeaponSwitch()`：後者還會清
...
export function dualShot(x, y){
  if(!state.dualWield) return false;
  if(state.over||state.transitioning||state.cutinPlaying) return false;
  /* ⚠⚠ **敵人死了照樣打得下去**（ver -1336，Ray 指定）：BR 中途擊殺時窗口不收，
     剩下的額度繼續打 —— 那幾發由 `enemyDamage` 自己 1:1 進 overkill 點數
  ...
  updateStatus();
  /* ══ 額度打完就收窗（ver -1338）══ 另一條出路是計時器到期。
     兩條都走 `endDual` → `onDualClosed`，由那一支決定是「掃殘磚」還是
     「收掉這一段 overkill」—— **收尾只有一條路**（鐵律 8）。
     ⚠ -1337 曾經在這裡自己分歧（敵死時直接叫 `autoClearOverkill`），
       於是同一件事有兩個入口；現在一律交給 `onDualClosed`。 */
...
/* 同時存活的碎屑上限（超過就少噴）。⚠ ver -1056 由 90 放寬到 **130**：散彈是
   **一次噴出 `hits` 顆**（最多 8），每顆十來顆火星就會頂到舊的上限，於是後面那幾顆
   彈丸只剩星芒沒有火星 —— 那看起來像「有的彈丸沒打中」。⚠ 這幾層都沒有 filter，
   多這幾十個元素的成本遠低於 -859 那一版（見 `glassShards` 的說明）。 */
const MZ_MAX=130;
let mzLive=0;
const mzPool=[];
function mzEl(cls){
...
/* ⚠ ver -1069：`claspSig`／`claspRetry` **回來了** —— Ray：「破防計的改動錯了，
   ...
   **副武器鈕維持 -1068 的 CSS flex 定位**（他要退的只有破防計那一半）。 */
let claspSig='';
...
function layoutClasp(){
  ...
  /* ⚠ ver -1069：血條右側的「讓位量」那一段拿掉了 —— 副武器鈕自 -1068 起是
     **flex 的子元素**，血條 `flex:1` 自己讓位（見 style.css 的 `#barsBlock`）。
     這裡只還需要 `btn` 來取它的 offsetParent（計量表的落點是它的鏡像）。 */
  ...
  /* ⚠ 鈕自 -1068 起是 `position:static`，`offsetParent` 不再是那塊面板 ——
     直接問 `#barsBlock`（它本來就是 -549 當初的定位錨）。 */
  const panel=document.getElementById('barsBlock');
  const par=panel ? panel.getBoundingClientRect() : {left:hr.x};
  ...
  /* ⚠ ver -1069：副武器鈕的位置與大小**不再由這裡寫** —— 那是 -1068 搬進 CSS 的
     （`--ws-size` ＋ flex），而 Ray 要退回的只有**破防計量表**這一半。 */
...
/* 連擊數的橫向擺位（唯一實作，鐵律 8）：錨在圓心，但 Ray：「可覆蓋月牙、
   **不可蓋過 HP 條**」—— 依當下字寬把右緣夾在血條左緣內。 */
function placeCombo(){
  if(!claspGeo || claspGeo.cx==null) return;
...
function withThumb(css){
  ...
  const faceW=(claspGeo && claspGeo.faceD ? claspGeo.faceD : 32);
...
/* 進場後多試幾拍（血條要排好才量得到）；視窗變了整組重量。
   ⚠ ver -1069 回到量測版之後這一支又有意義了（-1067 的重試也跟著回來）。 */
...
function enterOverkillFx(){
  ...
  /* ══⚠⚠⚠ **BR 中擊殺：窗口不收，改用瞄準點打完這一段 overkill**（ver -1337，
     ...
     ⚠⚠ **額度重設成「殘磚數」**（不是原本的 ×2）—— 這一段是 overkill 不是破防，
       份量由 Ray 定成一磚一發。
     ⚠ 兩個結束條件：**3 秒到**（由 **BR 自己那一支計時器**跑，見 `weapon.setDualBudget`）
       或 **額度打完**。兩條都是「BR 關窗」，收尾統一在 `onDualClosed`（唯一的收尾）。 */
  ...
  advanceExpectPastCleared();
  /* ══ BR 還開著 → **這一段由 BR 自己跑**（ver -1338，Ray：「BR 時優先走 BR，
     BR 的三秒跑完就結束」）══ 額度換成殘磚數、BR 的計時器重設成 overkill 的 3 秒，
     盤面維持鎖住。
     ⚠ **不要再 `armOverkillLimit()`**：那會變成兩支計時器同時在跑（BR 的與 overkill
       的），誰先到誰收尾 —— 讀起來就是「有時候 3 秒有時候 6 秒」。收尾統一在
...
/* ══ BR 窗口關掉的那一刻要做什麼 —— 判定只有這一支（ver -1338）══
   ...
   · 敵人已經死了（BR 中途擊殺）→ **直接收掉這一段 overkill**（連環碎裂 → 進下一隻）。
     BR 的計時器就是那 3 秒（見 `enterOverkillFx`），所以「BR 關窗」＝「這一段結束」，
     兩個結束條件（3 秒到／額度打完）都會走到這裡來 —— 收尾只有一條路（鐵律 8）。
   ⚠⚠ **不要把盤面交還給玩家點**：Ray 定的是「br ovk 時下方磚不可點」「不要解鎖」，
     這一段從頭到尾都不給點；殘磚由 `autoClearOverkill` 自己碎掉。
...
export function startGame(){
  ...
  /* ══⚠⚠ **`fullHp:true` ＝這一場無視持久 HP，滿血開場**（ver -1672，Ray：
     「強制夢魘化的那一場要**從滿血開始**…**只限這一場**」）══
     ⚠⚠⚠ 為什麼非要不可：惡夢化的**長度由發動時的血量決定**（ver -974：
       `niTotalMs = NI_MAX_SEC × (hp−1)/(max−1)`，滿血才有滿秒數）——
       而這一場是**劇本硬塞給玩家的**，他是在前面那一場被打得只剩幾滴血之後
       被推進來的。照持久 HP 開場的話，那一段惡夢化只會有一兩秒，
       **而畫面上看起來就只是「才剛發動就熔斷」**。
     ⚠ 它**只寫在這一張卡上**：這是劇本的例外，不是新的通則 ——
...
/* ══⚠⚠ **一鍵清盤 ＝ 秒殺**（ver -1428，Ray：「鎖血以後按清盤＝秒殺」）══
   ver -463 起這一支是「把這一盤點完」—— 一盤只打掉一盤的傷害，五盤的場次要按五次。
   Ray 要的是**一按就結束這一場**，所以清完盤之後**把剩下的血補掉**。
   ⚠ 走 `enemyDamage`（**與普攻同一條**，`src:'dev'` 不套武器倍率）——
     擊殺演出／overkill／連戰換敵／結算全部照舊。
     **不要自己把 `enemyHp` 設成 0**：那會繞過上面那一整套。
   ⚠ 這一支本來就只有 `body.testmode` 的那顆鈕叫得到（`#testClearBtn`）。 */
export function testClearBoard(){
  ...
  /* 清完盤還沒死就補掉剩下的血 ＝ **秒殺**（ver -1428，見上面的說明）。
     ⚠ 已經死了（或這一場結束了）就不要再打一發 —— 那會變成白白多一段 overkill。 */
  if(!state.over && state.enemyHp>0) enemyDamage(state.enemyHp, false, true, 'dev');
}
